...
const AdminPurchaseOrdersScreen: React.FC = () => {
  ...
  const {
    ...
    hasNextPage,
    hasPrevPage,
  } = usePaginatedFirestore<PurchaseOrder>("purchaseOrders", initialSort);
                ...
                <tbody>
                  {filteredPOs.map((po) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusPillClasses(
                            po.status
                          )}`}
                        >
                          {po.status}
              ...
              {filteredPOs.map((po) => (
                ...
                >
                  ...
                  <div className="mt-4 pt-4 border-t dark:border-slate-600 flex justify-between items-center">
                    ...
                    <button
                      onClick={() => navigate(`/purchase-orders/${po.id}`)}
                      className="text-admin-primary bg-admin-primary/10 font-semibold px-4 py-2 text-sm rounded-md"
                    >
                      عرض التفاصيل
                    </button>
                  </div>
