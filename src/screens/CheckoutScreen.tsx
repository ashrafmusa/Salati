...
import { calculateItemAndExtrasTotal, getOptimizedImageUrl } from '../utils/helpers';
import { SpinnerIcon } from '../assets/icons';

const CheckoutScreen: React.FC = () => {
  const { state, deliveryFee, getCartSubtotal, getDiscountDetails, getFinalTotal, clearCart } = useCart();
  const { user } = useAuth();
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const newOrderId = await db.runTransaction(async (transaction) => {
            const itemsToUpdate: { ref: firebase.firestore.DocumentReference, newStock: number }[] = [];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm">
              <h2 className="text-xl sm:text-2xl font-bold mb-4 text-slate-900 dark:text-slate-100">معلومات التوصيل</h2>
              ...
              <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
                {state.items.map(item => (
                  <div key={item.cartId} className="flex items-start gap-4">
                    ...
                    <p className="font-semibold text-slate-700 dark:text-slate-200">{calculateItemAndExtrasTotal(item, item.selectedExtras) * item.quantity} ج.س</p>
                  </div>
                ))}
              </div>
