    ...
    <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm">
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
                في الطريق مع السائق: {driver.name}
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                يمكنك التواصل معه مباشرة للاستفسار
              </p>
            </div>
            <a
              href={`tel:${driver.phone}`}
              className="flex items-center gap-2 px-3 py-2 bg-green-500 text-white font-semibold text-sm rounded-lg hover:bg-green-600 transition-colors"
            >
      ...
      <div className="mt-4 pt-2 border-t dark:border-slate-700">
        <button
          ...
          className="relative w-full text-center text-sm font-semibold text-primary hover:underline flex items-center justify-center gap-2"
        >
  ...
  useEffect(() => {
    if (order.customerHasUnreadMessages) {
      db.collection("orders")
        ...
        .catch((err) =>
          console.error("Failed to mark messages as read for customer:", err)
        );
    }
