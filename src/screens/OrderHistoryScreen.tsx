...
const OrderCard: React.FC<{ order: Order; onSupportClick: () => void; }> = ({ order, onSupportClick }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm">
            ...
            <div className="mt-4 pt-2 border-t dark:border-slate-700">
                <button onClick={onSupportClick} className="w-full text-center text-sm font-semibold text-primary hover:underline flex items-center justify-center gap-2">
            ...
            <div className="p-4 max-w-2xl mx-auto">
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {orders.map(order => (
                            <OrderCard key={order.id} order={order} onSupportClick={() => setSupportOrder(order)} />
                        ))}
                    </div>
                ) : (
