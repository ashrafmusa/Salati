        ...
        {loading ? (
          ...
          <>
            ...
            <div className="space-y-4 md:hidden">
                {logs.map(log => (
                    <div key={log.id} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm border dark:border-slate-700">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="font-bold text-slate-800 dark:text-slate-100">{log.action}</p>
                                <p className="text-sm text-slate-500 dark:text-slate-400">بواسطة: {log.adminName}</p>
                            </div>
                            <p className="text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                {new Date(log.timestamp).toLocaleDateString('ar-EG', { month:'short', day:'numeric', hour:'numeric', minute:'2-digit' })}
                            </p>
                        </div>
                        {log.details && (
