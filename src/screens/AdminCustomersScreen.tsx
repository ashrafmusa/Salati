    ...
    const unsubUsers = db
      ...
      .onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
        const fetchedUsers = snapshot.docs.map(
          (doc) => ({ id: doc.id, uid: doc.id, ...(doc.data() as any) } as User)
        );
              ...
              {usersToRender.map((user: PaginatedUser) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      handleSelectUser(user.uid, !selectedUserIds.has(user.uid))
                    }
                  >
                    ...
                    <div className="mt-2 text-sm grid grid-cols-2 gap-2">
                      <p>
                        <strong>الطلبات:</strong> {user.totalOrders}
                      </p>
                      <p>
                        <strong>المدفوعات:</strong>{" "}
                        {user.totalSpent.toLocaleString()} ج.س
                      </p>
                    </div>
                    {permissions.canChangeRole ||
                    permissions.canEdit ||
