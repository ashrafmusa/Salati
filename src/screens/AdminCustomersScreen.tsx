...
const AdminCustomersScreen: React.FC = () => {
    ...
    useEffect(() => {
        setLoading(true);
        const unsubUsers = db.collection('users').onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
            const fetchedUsers = snapshot.docs.map(doc => ({ id: doc.id, uid: doc.id, ...(doc.data() as any) } as User));
            setAllUsers(fetchedUsers);
        });

        const unsubOrders = db.collection('orders').onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
            setAllOrders(snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) } as Order)));
            setLoading(false); 
        }, () => setLoading(false));
    ...
    const customerStats = useMemo(() => {
        ...
        allOrders.forEach(order => {
            const current = stats.get(order.userId) || { totalOrders: 0, totalSpent: 0 };
    ...
    const filteredAndSortedUsers = useMemo(() => {
        ...
        return augmentedUsers.filter((user: PaginatedUser) =>
    ...
    const {
        ...
        hasNextPage,
        hasPrevPage,
    } = useClientSidePagination<PaginatedUser>(filteredAndSortedUsers, initialSort);
                ...
                <>
                    ...
                    <div className="overflow-x-auto hidden md:block">
                        <table className="w-full text-right">
                            <thead className="border-b-2 border-slate-100 dark:border-slate-700">
                                <tr>
                                    <th className="p-3"><input type="checkbox" onChange={handleSelectAll} checked={selectedUserIds.size === filteredAndSortedUsers.length && filteredAndSortedUsers.length > 0} /></th>
                                    <SortableHeader<PaginatedUser> label="المستخدم" sortKey="name" requestSort={requestSort} sortConfig={sortConfig} />
                                    <SortableHeader<PaginatedUser> label="إجمالي الطلبات" sortKey="totalOrders" requestSort={requestSort} sortConfig={sortConfig} />
                                    <SortableHeader<PaginatedUser> label="إجمالي المدفوعات" sortKey="totalSpent" requestSort={requestSort} sortConfig={sortConfig} />
                            ...
                            <tbody>
                                {usersToRender.map((user: PaginatedUser) => {
                                    ...
                                    <tr key={user.uid} className={`border-b dark:border-slate-700 transition-colors ${selectedUserIds.has(user.uid) ? 'bg-sky-100/50 dark:bg-sky-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>
                                        ...
                                        <td className="p-3 text-slate-600 dark:text-slate-300">{user.totalOrders}</td>
                                        <td className="p-3 text-slate-600 dark:text-slate-300">{(user.totalSpent || 0).toLocaleString()} ج.س</td>
                                        <td className="p-3">
                    ...
                    <div className="space-y-4 md:hidden">
                        {usersToRender.map((user: PaginatedUser) => {
                            const permissions = getManagementPermissions(user);
                            return (
                                <div key={user.uid} className={`bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border dark:border-slate-700 ${selectedUserIds.has(user.uid) ? 'ring-2 ring-admin-primary' : ''}`} onClick={() => handleSelectUser(user.uid, !selectedUserIds.has(user.uid))}>
                                    ...
                                    <div className="mt-2 text-sm grid grid-cols-2 gap-2">
                                        <p><strong>الطلبات:</strong> {user.totalOrders}</p>
                                        <p><strong>المدفوعات:</strong> {user.totalSpent.toLocaleString()} ج.س</p>
                                    </div>
                                    {permissions.canChangeRole || permissions.canEdit || permissions.canDelete ? (
                                        <div className="mt-3 pt-2 border-t dark:border-slate-600 space-y-2">
                                            {permissions.canChangeRole && (
                                                 <select
                                                    value={user.role}
                                                    onChange={(e) => { e.stopPropagation(); handleRoleChange(user.uid, e.target.value as User['role']); }}
                                                    onClick={(e) => e.stopPropagation()}
                                                    className={`w-full p-2 rounded text-sm border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700`}
                                                >
                                                    <option value="customer">Customer</option>
                                                    <option value="driver">Driver</option>
                                                    <option value="sub-admin">Sub-Admin</option>
                                            ...
                                            <div className="flex justify-end gap-4">
                                                {permissions.canEdit && <button onClick={(e) => { e.stopPropagation(); handleOpenEditModal(user); }} className="text-admin-primary font-semibold">تعديل</button>}
                                                {permissions.canDelete && <button onClick={(e) => { e.stopPropagation(); handleDeleteUser(user.uid, user.name); }} className="text-red-500 font-semibold">حذف</button>}
                                            </div>
                                        </div>
                                    ) : null}
                                </div>
