...
const AdminDriversScreen: React.FC = () => {
    ...
    const handleSaveDriver = async (driverToSave: Driver) => {
        ...
        try {
            ...
            await logAdminAction(adminUser, editingDriver ? 'Updated Driver' : 'Created Driver', `Name: ${driverToSave.name}`);
        } catch (error) {
    ...
    const handleDeleteDriver = async (driverId: string, driverName: string) => {
        if (window.confirm(`Are you sure you want to delete driver "${driverName}"?`)) {
            try {
                ...
                await logAdminAction(adminUser, 'Deleted Driver', `Name: ${driverName}`);
    ...
    return (
        <div className="h-full flex flex-col bg-white dark:bg-slate-800 p-4 md:p-6 rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">قائمة السائقين</h2>
                <button 
            ...
            <div className="flex-grow overflow-y-auto">
                {loading ? <TableSkeleton /> : drivers.length > 0 ? (
                <>
                    {/* Desktop Table View */}
                    <div className="overflow-x-auto hidden md:block">
                        <table className="w-full text-right">
                            <thead className="border-b-2 border-slate-100 dark:border-slate-700">
                                <tr>
                                    <SortableHeader<Driver> label="اسم السائق" sortKey="name" requestSort={requestSort} sortConfig={sortConfig} />
                                    <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">رقم الهاتف</th>
                                    <SortableHeader<Driver> label="الحالة" sortKey="status" requestSort={requestSort} sortConfig={sortConfig} />
                                    <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">إجراءات</th>
                                </tr>
                            </thead>
                            <tbody>
                                {drivers.map((driver, index) => (
                                    <tr key={driver.id} className={`border-b dark:border-slate-700 transition-colors ${index % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'} hover:bg-sky-100/50 dark:hover:bg-sky-900/20`}>
                                        <td className="p-3 font-medium text-slate-700 dark:text-slate-200">{driver.name}</td>
                                        <td className="p-3 text-slate-600 dark:text-slate-300" dir="ltr">{driver.phone}</td>
                                        <td className="p-3 text-slate-600 dark:text-slate-300">{driver.status}</td>
                    ...
                    <div className="space-y-4 md:hidden">
                        {drivers.map(driver => (
                            <div key={driver.id} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm border dark:border-slate-700">
                            <div className="flex justify-between items-start">
                                    <div>
                                        <p className="font-bold text-slate-800 dark:text-slate-100">{driver.name}</p>
                                        <p className="text-sm text-slate-500 dark:text-slate-400" dir="ltr">{driver.phone}</p>
                                    </div>
                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                                        driver.status === 'Available' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 
                                        driver.status === 'On-Delivery' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300' : 
                                ...
                                <div className="flex justify-end gap-4 mt-4 pt-2 border-t dark:border-slate-600">
                                    <button onClick={() => handleOpenModal(driver)} className="text-admin-primary font-semibold">تعديل</button>
                                    <button onClick={() => handleDeleteDriver(driver.id, driver.name)} className="text-red-500 font-semibold">حذف</button>
                                </div>
                            </div>
                        ))}
                    </div>
