...
const AdminOrdersScreen: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = db.collection("drivers").onSnapshot((snapshot) => {
      setDrivers(
        snapshot.docs.map(
          (doc) => ({ id: doc.id, ...(doc.data() as any) } as Driver)
        )
      );
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    if (viewOrderFromUrl && paginatedOrders.length > 0) {
      const orderToView = paginatedOrders.find(
        (o) => o.id === viewOrderFromUrl
      );
  ...
  const updateOrderWithLog = async (
    orderId: string,
    updateData: Partial<Omit<AdminOrder, "id">>,
    logMessage: string
  ) => {
    if (!adminUser) return;
    try {
                      ...
                      >
                        ...
                        <td className="p-3">
                          <select
                            ...
                            className={`p-1.5 w-40 text-sm font-semibold rounded-lg border-2 ${select}`}
                          >
                            {Object.values(OrderStatus).map((s) => (
                              <option key={s} value={s}>
                                {s}
                    ...
                    <div className="mt-4 space-y-3">
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full p-2 text-sm font-semibold rounded-lg border-2 ${select}`}
                        >
                          {Object.values(OrderStatus).map((s) => (
                            <option key={s} value={s}>
                              {s}
                      ...
                      <div>
                        ...
                        <select
                          value={order.driverId || ""}
                          onChange={(e) =>
                            handleDriverAssign(order.id, e.target.value || null)
                          }
                          className="w-full p-2 rounded text-sm bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600"
                        >
                          <option value="">لم يتم التعيين</option>
                          {drivers.map((d) => (
                            <option key={d.id} value={d.id}>
