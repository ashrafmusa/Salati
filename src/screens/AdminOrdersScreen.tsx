...
const AdminOrdersScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = driversQuery.onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
    ...
    const filters = useMemo(() => {
        ...
        if (statusFilter && statusFilter !== 'all') {
            q.push(['status', '==', statusFilter]);
        }
        if (deliveryMethodFilter && deliveryMethodFilter !== 'all') {
            q.push(['deliveryMethod', '==', deliveryMethodFilter]);
        }
        if (paymentStatusFilter && paymentStatusFilter !== 'all') {
            q.push(['paymentStatus', '==', paymentStatusFilter]);
        }
        return q;
    ...
    const handleBulkStatusChange = async (newStatus: OrderStatus) => {
        ...
        try {
            await batch.commit();
            logAdminAction(adminUser, 'Bulk Order Status Change', `${selectedOrderIds.size} orders -> ${newStatus}`);
    ...
    const handleStatusChange = async (orderId: string, newStatus: OrderStatus) => {
        ...
        try {
            ...
            logAdminAction(adminUser, 'Order Status Changed', `Order #${orderId.slice(0, 5)}: ${oldStatus} -> ${newStatus}`);
    ...
    const handleDriverAssign = async (orderId: string, driverId: string | null) => {
        try {
            await db.collection('orders').doc(orderId).update({ driverId });
            const driverName = drivers.find(d => d.id === driverId)?.name || 'Unassigned';
            logAdminAction(adminUser, 'Assigned Driver', `Order #${orderId.slice(0, 5)} -> ${driverName}`);
    ...
    const handleDeleteOrder = (order: AdminOrder) => {
        setConfirmationState({
            isOpen: true,
            title: "تأكيد الحذف",
            message: `هل أنت متأكد من رغبتك في حذف الطلب #${order.id.slice(0, 7).toUpperCase()}؟ لا يمكن التراجع عن هذا الإجراء.`,
            isDestructive: true,
            onConfirm: async () => {
                try {
        ...
        <>
            <div className="h-full flex flex-col bg-white dark:bg-slate-800 p-4 md:p-6 rounded-lg shadow-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as OrderStatus | 'all')} className="w-full p-2 border rounded-lg bg-white dark:bg-slate-700 dark:border-slate-600">
                        <option value="all">كل الحالات</option>
                        {Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <select value={deliveryMethodFilter} onChange={e => setDeliveryMethodFilter(e.target.value as 'all' | 'delivery' | 'pickup')} className="w-full p-2 border rounded-lg bg-white dark:bg-slate-700 dark:border-slate-600">
                        <option value="all">كل طرق الاستلام</option>
                        <option value="delivery">توصيل</option>
                        <option value="pickup">استلام من المتجر</option>
                    </select>
                    <select value={paymentStatusFilter} onChange={e => setPaymentStatusFilter(e.target.value as 'all' | 'paid' | 'unpaid')} className="w-full p-2 border rounded-lg bg-white dark:bg-slate-700 dark:border-slate-600">
                ...
                <div className="flex-grow overflow-y-auto">
                    ...
                    <div className="overflow-x-auto hidden md:block">
                        <table className="w-full text-right">
                            <thead className="border-b-2 border-slate-100 dark:border-slate-700">
                                <tr>
                                    <th className="p-3"><input type="checkbox" onChange={handleSelectAll} checked={selectedOrderIds.size === filteredOrders.length && filteredOrders.length > 0} /></th>
                                    <SortableHeader<AdminOrder> label="رقم الطلب" sortKey="id" requestSort={requestSort} sortConfig={sortConfig} />
                                    <th className="p-3 text-sm font-semibold text-slate-500">العميل</th>
                                    <SortableHeader<AdminOrder> label="الإجمالي" sortKey="total" requestSort={requestSort} sortConfig={sortConfig} />
                                    <th className="p-3 text-sm font-semibold text-slate-500">الحالة</th>
                                    <th className="p-3 text-sm font-semibold text-slate-500">السائق</th>
                                    <th className="p-3 text-sm font-semibold text-slate-500">إجراءات</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredOrders.map((order) => {
                                    const { select } = getStatusPillClasses(order.status || OrderStatus.Preparing);
                                    return (
                                        <tr key={order.id} className={`border-b dark:border-slate-700 transition-colors ${selectedOrderIds.has(order.id) ? 'bg-sky-100/50 dark:bg-sky-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>
                                            <td className="p-3"><input type="checkbox" checked={selectedOrderIds.has(order.id)} onChange={(e) => handleSelectOrder(order.id, e.target.checked)} /></td>
                                            ...
                                            <td className="p-3">
                                                <select value={order.status} onChange={e => handleStatusChange(order.id, e.target.value as OrderStatus)} className={`p-1.5 w-40 text-sm font-semibold rounded-lg border-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-800 focus:ring-admin-primary ${select}`}>
                                                    {Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}
                                                </select>
                                            </td>
                                            <td className="p-3">
                                            ...
                                            <td className="p-3 space-x-4 space-x-reverse">
                                                <button onClick={() => setSelectedOrder(order)} className="text-admin-primary hover:underline">التفاصيل</button>
                                                <button onClick={() => handleDeleteOrder(order)} className="text-red-500 hover:underline">حذف</button>
                                            </td>
                                        </tr>
                                    );
                                })}
                    ...
                    <div className="space-y-4 md:hidden">
                        {filteredOrders.map((order) => (
                            <div key={order.id} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm border dark:border-slate-700">
                                <div className="flex items-start gap-2">
                                    <input type="checkbox" className="mt-1" checked={selectedOrderIds.has(order.id)} onChange={(e) => handleSelectOrder(order.id, e.target.checked)} />
                                    <div className="flex-grow">
                                        ...
                                        <div className="mt-4 pt-4 border-t dark:border-slate-700 space-y-3">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <select value={order.status} onChange={e => handleStatusChange(order.id, e.target.value as OrderStatus)} className={`p-1 w-36 rounded text-xs border focus:ring-admin-primary ${getStatusPillClasses(order.status).select}`}>
                                                    {Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}
                                                </select>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <label className="text-sm font-medium">السائق:</label>
                                                <select value={order.driverId || ''} onChange={e => handleDriverAssign(order.id, e.target.value || null)} className="p-1 w-36 rounded text-xs bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600">
                                                    <option value="">لم يتم التعيين</option>
                                                    {drivers.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                                                </select>
                                            </div>
                                            <div className="flex justify-end gap-4 pt-2">
                                                <button onClick={() => setSelectedOrder(order)} className="text-admin-primary font-semibold">التفاصيل</button>
                                                <button onClick={() => handleDeleteOrder(order)} className="text-red-500 font-semibold">حذف</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            {selectedOrder && (
                <OrderDetailsModal order={selectedOrder} onClose={() => setSelectedOrder(null)} drivers={drivers} />
            )}
