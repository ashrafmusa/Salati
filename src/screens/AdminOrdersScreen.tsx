...
const getStatusPillClasses = (status: OrderStatus) => {
    switch (status) {
        case OrderStatus.Delivered: return { dot: 'bg-status-delivered', select: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 border-green-500/50 dark:border-green-500/70'};
        case OrderStatus.Preparing: return { dot: 'bg-status-preparing', select: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300 border-yellow-500/50 dark:border-yellow-500/70' };
        ...
        case OrderStatus.OutForDelivery: return { dot: 'bg-status-delivering', select: 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300 border-blue-500/50 dark:border-blue-500/70' };
        case OrderStatus.Cancelled: return { dot: 'bg-status-cancelled', select: 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300 border-red-500/50 dark:border-red-500/70' };
...
const AdminOrdersScreen: React.FC = () => {
    ...
    const handleStatusChange = (order: AdminOrder, newStatus: OrderStatus) => {
        ...
        const performUpdate = (status: OrderStatus, driverId: string | null = order.driverId) => {
        ...
        if (newStatus === OrderStatus.Delivered || newStatus === OrderStatus.Cancelled) {
            setConfirmationState({
                isOpen: true,
                title: `Confirm Order ${newStatus === OrderStatus.Delivered ? 'Delivery' : 'Cancellation'}`,
                message: `Are you sure you want to mark order #${order.id.slice(0, 7).toUpperCase()} as ${newStatus}?`,
                onConfirm: () => {
            ...
            setTimeout(() => {
                const select = document.getElementById(`status-${order.id}`) as HTMLSelectElement;
        ...
        if (newStatus === OrderStatus.OutForDelivery && !order.driverId) {
            ...
            if (availableDriver) {
                ...
                alert("No available drivers to assign. Please assign a driver manually.");
                 setTimeout(() => {
                    const select = document.getElementById(`status-${order.id}`) as HTMLSelectElement;
                    if (select) select.value = order.status;
                }, 0);
    ...
    const handleDeleteOrder = async (orderId: string) => {
        setConfirmationState({
            ...
            message: `Are you sure you want to permanently delete order #${orderId.slice(0, 7).toUpperCase()}? This action cannot be undone.`,
            isDestructive: true,
            onConfirm: async () => {
                try {
    ...
    const inputSelectClasses = "p-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-admin-primary focus:outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm";
            ...
            <div className="bg-white dark:bg-slate-800 p-4 md:p-6 rounded-lg shadow-md">
                    ...
                    <table className="w-full text-right">
                        <thead className="border-b-2 border-slate-100 dark:border-slate-700">
                            <tr>
                                ...
                                <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">حالة الدفع</th>
                                <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">حالة الطلب</th>
                                <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">السائق</th>
                        ...
                        <tbody>
                            {filteredOrders.map((order, index) => {
                                ...
                                return (
                                <tr key={order.id} className={`border-b dark:border-slate-700 transition-colors ${index % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'} hover:bg-sky-100/50 dark:hover:bg-sky-900/20`}>
                                    <td className="p-3 font-medium text-slate-700 dark:text-slate-200">{order.id.slice(0, 7).toUpperCase()}</td>
                                    <td className="p-3 text-slate-600 dark:text-slate-300">{order.deliveryInfo.name}</td>
                                    <td className="p-3 text-slate-600 dark:text-slate-300">{order.total} ج.س</td>
                                    <td className="p-3">
                                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${order.paymentStatus === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300'}`}>
                                            {order.paymentStatus === 'paid' ? 'مدفوع' : 'غير مدفوع'}
                                        </span>
                                    </td>
                                    <td className="p-3 w-48">
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2.5 h-2.5 rounded-full ${statusClasses.dot}`}></span>
                                            <select
                                                id={`status-${order.id}`}
                                                value={order.status} 
                                    ...
                                    <td className="p-3 w-40">
                                        <select 
                                            ...
                                            onChange={e => handleDriverChange(order.id, e.target.value || null)}
                ...
                <div className="space-y-4 md:hidden">
                    {filteredOrders.map(order => {
                        const statusClasses = getStatusPillClasses(order.status);
                        return (
                        <div key={order.id} className={`bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm border-l-4 ${statusClasses.dot.replace('bg-', 'border-')}`}>
                           <div className="flex justify-between items-start mb-3">
                                <div>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">طلب #{order.id.slice(0, 7).toUpperCase()}</p>
                                    <p className="font-bold text-slate-800 dark:text-slate-100">{order.deliveryInfo.name}</p>
                                </div>
                                <p className="text-xl font-bold text-primary dark:text-green-400">{order.total} ج.س</p>
                            </div>
                            
                            <div className="pt-3 border-t dark:border-slate-700">
                                <div className="mb-3">
                                    <label className="text-xs text-slate-500 dark:text-slate-400 block mb-1">حالة الدفع:</label>
                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${order.paymentStatus === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300'}`}>
                                      {order.paymentStatus === 'paid' ? 'مدفوع' : 'غير مدفوع'}
                                    </span>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        <label className="text-xs text-slate-500 dark:text-slate-400 block mb-1">حالة الطلب:</label>
                                        <select
                                    ...
                                    <div>
                                        <label className="text-xs text-slate-500 dark:text-slate-400 block mb-1">السائق:</label>
                                        <select 
                                            value={order.driverId || ''} 
                                            onChange={e => handleDriverChange(order.id, e.target.value || null)}
