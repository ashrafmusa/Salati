...
import { Offer, StoreProduct, Discount, Item, Bundle } from '../types';
...
const OfferFormModal: React.FC<{
  ...
  products: StoreProduct[];
    ...
    const handleDiscountChange = (field: string, value: string | number) => {
        setFormData(prev => {
            const newDiscount: any = { ...(prev.discount || {}) };
            newDiscount[field] = value;

            if (field === 'type') {
                delete newDiscount.target;
                if (value === 'buyXgetY') {
                    newDiscount.buyQuantity = 10;
                    newDiscount.getQuantity = 1;
                    newDiscount.appliesTo = 'product';
                    delete newDiscount.value;
                } else {
                    newDiscount.appliesTo = 'all';
                    newDiscount.value = 10;
                    delete newDiscount.buyQuantity;
                    delete newDiscount.getQuantity;
                }
            }
            ...
            return { ...prev, discount: newDiscount as Discount };
        });
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <fieldset className="border dark:border-slate-600 p-4 rounded-md space-y-3">
                        ...
                        <select value={discount?.type} onChange={e => handleDiscountChange('type', e.target.value)} className={inputClasses}>
                            ...
                            <option value="buyXgetY">اشتر X واحصل على Y مجاناً</option>
                        </select>
                        
                        {discount?.type !== 'buyXgetY' && (
                            <>
                                <div className="grid grid-cols-2 gap-4">
                                    <input type="number" value={(discount as any)?.value || ''} onChange={e => handleDiscountChange('value', Number(e.target.value))} className={inputClasses} placeholder="قيمة الخصم" required />
                        ...
                        {discount?.type === 'buyXgetY' && (
                            <>
                                <div className="grid grid-cols-3 gap-4 items-center">
                                    <input type="number" value={(discount as any).buyQuantity || ''} onChange={e => handleDiscountChange('buyQuantity', Number(e.target.value))} className={inputClasses} placeholder="اشترِ (كمية)" required />
                                    <span>واحصل على</span>
                                    <input type="number" value={(discount as any).getQuantity || ''} onChange={e => handleDiscountChange('getQuantity', Number(e.target.value))} className={inputClasses} placeholder="مجاناً (كمية)" required />
                                </div>
                                <select value={discount?.target} onChange={e => handleDiscountChange('target', e.target.value)} className={inputClasses} required>
                                    <option value="">اختر المنتج المستهدف</option>
                                    {products.filter(p => p.type === 'item').map(p => <option key={p.id} value={p.id}>{p.arabicName}</option>)}
                                </select>
                            </>
                        )}
                    </fieldset>
...
const DiscountDetails: React.FC<{ discount?: Discount; products: StoreProduct[], className?: string }> = ({ discount, products, className="" }) => {
    if (!discount) {
    ...
    switch (discount.type) {
        case 'buyXgetY':
            const productNameBOGO = products.find(p => p.id === discount.target)?.arabicName || discount.target;
            description = <><span className="text-lg text-primary font-bold">اشترِ {discount.buyQuantity} واحصل على {discount.getQuantity}</span> مجاناً</>;
            targetText = <>منتج: <span className="font-bold">{productNameBOGO}</span></>;
            break;
        case 'percentage':
        case 'fixed':
            const typeText = discount.type === 'percentage' ? '%' : 'ج.س';
            description = <><span className="text-lg text-primary font-bold">{(discount as any).value}{typeText}</span> خصم</>;
    ...
    return (
        <div className={`bg-slate-100 dark:bg-slate-700/50 p-3 rounded-lg text-sm ${className}`}>
             <p className="font-semibold text-slate-800 dark:text-slate-200">{description}</p>
             <p className="text-slate-600 dark:text-slate-300">يطبق على: {targetText}</p>
        </div>
    );
};
...
const AdminOffersScreen: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAllProducts = async () => {
             ...
             const items = itemsSnapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any), type: 'item' } as Item));
             const bundles = bundlesSnapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any), type: 'bundle' } as Bundle));
             setProducts([...items, ...bundles]);
        };
        fetchAllProducts();
    }, []);

    const initialSort = useMemo(() => ({ key: 'expiryDate' as const, direction: 'descending' as const }), []);
    ...
    const handleSaveOffer = async (offerToSave: Offer) => {
        ...
        try {
            ...
            await logAdminAction(adminUser, editingOffer ? 'Updated Offer' : 'Created Offer', `Title: ${offerToSave.title}`);
        } catch (error) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await logAdminAction(adminUser, 'Deleted Offer', `Title: ${offerToDelete.title}`);
        ...
        <>
            <div className="h-full flex flex-col bg-white dark:bg-slate-800 p-4 md:p-6 rounded-lg shadow-md">
                ...
                <div className="flex-grow overflow-y-auto">
                    {loading ? <TableSkeleton /> : filteredOffers.length > 0 ? (
                        <>
                            {/* Desktop Table View */}
                            <div className="overflow-x-auto hidden md:block">
                                <table className="w-full text-right">
                                    <thead className="border-b-2 border-slate-100 dark:border-slate-700">
                                        <tr>
                                            <SortableHeader<Offer> label="العرض" sortKey="title" requestSort={requestSort} sortConfig={sortConfig} />
                                            <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">تفاصيل الخصم</th>
                                            <SortableHeader<Offer> label="تاريخ الانتهاء" sortKey="expiryDate" requestSort={requestSort} sortConfig={sortConfig} />
                                    ...
                                    <tbody>
                                        {filteredOffers.map(offer => {
                                            ...
                                            return (
                                            <tr key={offer.id} className="border-b dark:border-slate-700 hover:bg-sky-100/50 dark:hover:bg-sky-900/20 align-middle">
                                                <td className="p-3">
                                                    <div className="flex items-center gap-3">
                                                        <img src={getOptimizedImageUrl(offer.imageUrl, 100)} alt={offer.title} className="w-16 h-10 rounded-md object-cover"/>
                                                        <span className="font-medium text-slate-700 dark:text-slate-200">{offer.title}</span>
                                                    </div>
                                                </td>
                                                <td className="p-3">
                                                    <DiscountDetails discount={offer.discount} products={products} className="p-2" />
                                                </td>
                                                <td className="p-3 text-slate-600 dark:text-slate-300">
                                                    {new Date(offer.expiryDate).toLocaleDateString('ar-EG')}
                                                </td>
                                                <td className="p-3">
                                                    <span className={`px-3 py-1 text-xs font-bold rounded-full ${isExpired ? 'bg-red-200 text-red-800 dark:bg-red-900/50 dark:text-red-300' : 'bg-green-200 text-green-800 dark:bg-green-900/50 dark:text-green-300'}`}>
                                                        {isExpired ? 'منتهي' : 'نشط'}
                                                    </span>
                                                </td>
                                                <td className="p-3 space-x-4 space-x-reverse">
                                                    <button onClick={() => { setEditingOffer(offer); setIsModalOpen(true); }} className="text-admin-primary hover:underline">تعديل</button>
                                                    <button onClick={() => setOfferToDelete(offer)} className="text-red-500 hover:underline">حذف</button>
                                                </td>
                                            </tr>
                                        )})}
                                    </tbody>
                        ...
                        <AdminEmptyState
                            icon={GiftIcon}
                            title={searchTerm ? 'لا توجد عروض مطابقة' : 'لا توجد عروض بعد'}
                            message={searchTerm ? 'حاول البحث بكلمة أخرى.' : 'ابدأ بإنشاء عرض ترويجي جديد.'}
                            buttonText="إضافة عرض"
                            onButtonClick={() => { setEditingOffer(null); setIsModalOpen(true); }}
                        />
                    )}
