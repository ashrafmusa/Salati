              ...
              <>
                ...
                {discount?.appliesTo === "category" && (
                  <select
                    ...
                    onChange={(e) =>
                      handleDiscountChange("target", e.target.value)
                    }
                    className={inputClasses}
                    required
                  >
            ...
            {discount?.type === "buyXgetY" && (
              <>
                <div className="grid grid-cols-3 gap-4 items-center">
                  <input
                    type="number"
                    value={(discount as any).buyQuantity || ""}
                    onChange={(e) =>
                    ...
                    className={inputClasses}
                    placeholder="اشترِ (كمية)"
                    required
                  />
                  <span>واحصل على</span>
                  <input
                    type="number"
                    value={(discount as any).getQuantity || ""}
                    onChange={(e) =>
                      handleDiscountChange(
                        "getQuantity",
                        Number(e.target.value)
...
const DiscountDetails: React.FC<{
  ...
  switch (discount.type) {
    case "buyXgetY":
      const productNameBOGO =
        products.find((p) => p.id === discount.target)?.arabicName ||
        discount.target;
      description = (
        <>
          <span className="text-lg text-primary font-bold">
            اشترِ {discount.buyQuantity} واحصل على {discount.getQuantity}
          </span>{" "}
          مجاناً
        </>
      );
      targetText = (
        <>
          منتج: <span className="font-bold">{productNameBOGO}</span>
        </>
      );
      break;
    case "percentage":
  ...
  useEffect(() => {
    const fetchAllProducts = async () => {
      ...
      const items = itemsSnapshot.docs.map(
        (doc) => ({ id: doc.id, ...(doc.data() as any), type: "item" } as Item)
      );
      const bundles = bundlesSnapshot.docs.map(
        (doc) =>
          ({ id: doc.id, ...(doc.data() as any), type: "bundle" } as Bundle)
      );
  ...
  const handleSaveOffer = async (offerToSave: Offer) => {
    ...
    try {
      ...
      await logAdminAction(
        adminUser,
        editingOffer ? "Updated Offer" : "Created Offer",
