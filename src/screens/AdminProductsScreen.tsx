          ...
          {isAddDropdownOpen && (
            <div className="absolute left-0 mt-2 w-48 bg-white dark:bg-slate-700 rounded-md shadow-lg z-10">
              <button
                ...
                className="block w-full text-right px-4 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-600"
              >
                إضافة صنف جديد
              </button>
              <button
          ...
          <>
            ...
            <div className="space-y-4 md:hidden">
              {filteredProducts.map((product) => (
                <div
                  key={product.id}
                  className={`bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg shadow-sm border dark:border-slate-700 flex gap-3 ${
                    selectedProductIds.has(product.id)
                ...
                >
                  <img
                    src={getOptimizedImageUrl(product.imageUrl, 200)}
                    alt={product.arabicName}
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="flex justify-end gap-4 mt-2">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenModal(product.type, product);
                        }}
                        className="text-admin-primary font-semibold text-sm"
                      >
                        تعديل
                      </button>
                      <button
