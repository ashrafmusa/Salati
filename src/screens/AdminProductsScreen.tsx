...
const AdminProductsScreen: React.FC = () => {
    ...
    const ProductTypeBadge: React.FC<{ type: 'item' | 'bundle' }> = ({ type }) => (
        <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${type === 'item' ? 'bg-sky-100 text-sky-800 dark:bg-sky-900/50 dark:text-sky-300' : 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300'}`}>
            {type === 'item' ? 'صنف' : 'حزمة'}
        </span>
    );
             ...
             <div ref={addDropdownRef} className="relative">
                ...
                {isAddDropdownOpen && (
                    <div className="absolute left-0 mt-2 w-48 bg-white dark:bg-slate-700 rounded-md shadow-lg z-10">
                        <button onClick={() => { handleOpenModal('item'); setIsAddDropdownOpen(false); }} className="block w-full text-right px-4 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-600">إضافة صنف جديد</button>
                        <button onClick={() => { handleOpenModal('bundle'); setIsAddDropdownOpen(false); }} className="block w-full text-right px-4 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-600">إنشاء حزمة جديدة</button>
                    </div>
                )}
            </div>
        </div>
          ...
          {loading ? <TableSkeleton /> : (
            <>
                ...
                <div className="overflow-x-auto hidden md:block">
                    <table className="w-full text-right">
                        ...
                        {filteredProducts.map(product => (
                            <tr key={product.id} className={`border-b dark:border-slate-700 ${selectedProductIds.has(product.id) ? 'bg-sky-100/50 dark:bg-sky-900/30' : ''}`}>
                ...
                <div className="space-y-4 md:hidden">
                    {filteredProducts.map(product => (
                        <div key={product.id} className={`bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg shadow-sm border dark:border-slate-700 flex gap-3 ${selectedProductIds.has(product.id) ? 'ring-2 ring-admin-primary' : ''}`} onClick={() => handleSelectProduct(product.id, !selectedProductIds.has(product.id))}>
                            <img src={getOptimizedImageUrl(product.imageUrl, 200)} alt={product.arabicName} className="w-20 h-20 rounded-md object-cover" />
                            <div className="flex-grow">
                                <div className="flex justify-between items-start">
                                    <p className="font-bold text-slate-800 dark:text-slate-100">{product.arabicName}</p>
                                    <ProductTypeBadge type={product.type} />
                                </div>
                                <p className="text-sm text-slate-500 dark:text-slate-400">{product.category}</p>
                                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300 mt-1">المخزون: {product.stock}</p>
                                <div className="flex justify-end gap-4 mt-2">
                                    <button onClick={(e) => { e.stopPropagation(); handleOpenModal(product.type, product); }} className="text-admin-primary font-semibold text-sm">تعديل</button>
                                    <button onClick={(e) => { e.stopPropagation(); setProductToDelete(product); }} className="text-red-500 font-semibold text-sm">حذف</button>
                                </div>
                            </div>
                        </div>
                    ))}
