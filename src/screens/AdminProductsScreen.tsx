...
const BundleFormModal: React.FC<{ 
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (formData.arabicName && formData.category && formData.imageUrl && formData.contents && formData.contents.length > 0) {
            onSave(formData as Bundle);
        } else {
            showToast('Please fill all required fields, upload an image, and add at least one item.', 'error');
        }
    };
            ...
            <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            <textarea name="description" placeholder="الوصف" value={formData.description || ''} onChange={handleChange} rows={3} className={inputClasses}></textarea>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">صورة الحزمة</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t dark:border-slate-700">
                        <div>
                             ...
                             <div className="h-60 overflow-y-auto border dark:border-slate-700 rounded-md p-2 space-y-1">
                                {filteredItems.map(item => (
                                    <button type="button" key={item.id} onClick={() => addContentItem(item)} className="w-full text-right p-2 rounded hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-between gap-4">
                                        <span className="flex-grow">{item.arabicName}</span>
                                        <span className="text-sm text-slate-500 flex-shrink-0">{item.price} ج.س</span>
                                    </button>
                                ))}
                             </div>
                        </div>
                        <div>
                             ...
                             <div className={`h-60 overflow-y-auto border dark:border-slate-700 rounded-md p-2 ${ (formData.contents || []).length > 0 ? 'space-y-2' : 'flex items-center justify-center'}`}>
                                {(formData.contents || []).map((content, index) => {
                                    const item = allItems.find(i => i.id === content.itemId);
                                    return (
                                        <div key={index} className="flex items-center justify-between p-1 bg-slate-50 dark:bg-slate-800 rounded">
                                            <span className="text-sm font-medium flex-1">{item?.arabicName}</span>
                                ...
                                {(formData.contents || []).length === 0 && <p className="text-sm text-slate-400">لم يتم إضافة أصناف</p>}
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end pt-4 border-t dark:border-slate-700 mt-4 gap-4">
                        ...
                        <button type="submit" className="px-6 py-2 bg-admin-primary text-white rounded-md hover:bg-admin-primary-hover flex items-center justify-center w-24" disabled={isSaving || isUploading}>
...
const AdminBundlesScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        unsubs.push(onSnapshot(itemsQuery, snapshot => {
    ...
    const filteredBundles = useMemo(() => {
        ...
        return paginatedBundles.filter(bundle =>
    ...
    const itemsInStock = useMemo(() => items.filter(item => item.stock > 0), [items]);
    ...
    const handleCreateBundleFromIdea = (idea: BundleIdea) => {
        const itemsInBundle = items.filter(item => idea.itemNames.includes(item.arabicName));
    ...
    return (
        ...
        <div className="h-full flex flex-col bg-white dark:bg-slate-800 p-4 md:p-6 rounded-lg shadow-md">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 self-start sm:self-center">إدارة الحزم</h2>
            ...
            <div className="flex-grow overflow-y-auto">
                ...
                <>
                        ...
                        <table className="w-full text-right">
                            <thead>
                                <tr className="border-b-2 border-slate-100 dark:border-slate-700">
                                    <SortableHeader<Bundle> label="اسم الحزمة" sortKey="arabicName" requestSort={requestSort} sortConfig={sortConfig} />
                                    <SortableHeader<Bundle> label="الفئة" sortKey="category" requestSort={requestSort} sortConfig={sortConfig} />
                                    <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">السعر المحسوب</th>
                                    <SortableHeader<Bundle> label="المخزون" sortKey="stock" requestSort={requestSort} sortConfig={sortConfig} />
                                    <th className="p-3 text-sm font-semibold text-slate-500 dark:text-slate-400">إجراءات</th>
                                </tr>
                            </thead>
                            <tbody>
                                {bundlesWithPrices.map(bundle => (
                                    <tr key={bundle.id} className="border-b dark:border-slate-700 hover:bg-sky-100/50 dark:hover:bg-sky-900/20 align-middle">
                                        <td className="p-3">
                                            <div className="flex items-center gap-3">
                                                <img src={getOptimizedImageUrl(bundle.imageUrl, 100)} alt={bundle.arabicName} className="w-12 h-12 rounded-md object-cover"/>
                                                <span className="font-medium text-slate-700 dark:text-slate-200">{bundle.arabicName}</span>
                                            </div>
                                        </td>
                                        <td className="p-3 text-slate-600 dark:text-slate-300">{bundle.category}</td>
                                        <td className="p-3 text-slate-600 dark:text-slate-300">{bundle.calculatedPrice} ج.س</td>
                    ...
                    <div className="space-y-4 md:hidden">
                        {bundlesWithPrices.map(bundle => (
                            <div key={bundle.id} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm border dark:border-slate-700">
                                <div className="flex items-start gap-4">
                                    <img src={getOptimizedImageUrl(bundle.imageUrl, 150)} alt={bundle.arabicName} className="w-20 h-20 rounded-md object-cover flex-shrink-0" />
                                    <div className="flex-grow">
                                        <p className="font-bold text-lg text-slate-800 dark:text-slate-100">{bundle.arabicName}</p>
                                        <p className="text-sm text-slate-500 dark:text-slate-400">{bundle.category}</p>
                                    </div>
                                </div>
                                <div className="mt-4 pt-4 border-t dark:border-slate-700 space-y-3">
                                    <div className="grid grid-cols-2 gap-4 text-center">
                                        <div>
                                            ...
                                            <p className="font-semibold text-slate-700 dark:text-slate-200">{bundle.calculatedPrice} ج.س</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-slate-500 dark:text-slate-400">المخزون</p>
                                            <p className="font-semibold text-slate-700 dark:text-slate-200">{bundle.stock}</p>
                                        </div>
                                    </div>
                                    <div className="flex justify-end gap-4 pt-2">
                                        <button onClick={() => { setEditingBundle(bundle); setIsModalOpen(true); }} className="text-admin-primary font-semibold">تعديل</button>
                                        <button onClick={() => setBundleToDelete(bundle)} className="text-red-500 font-semibold">حذف</button>
                                    </div>
                                </div>
                            </div>
                        ))}
