...
const DriverDashboardScreen: React.FC = () => {
    ...
    const handleAcceptOrder = async (order: AdminOrder) => {
        ...
        try {
            ...
            alert(error.message || "Failed to accept order. It may have been taken by another driver.");
        } finally {
    ...
    const TabButton: React.FC<{ tab: ActiveTab, label: string, count: number }> = ({ tab, label, count }) => (
        ...
        >
            {label} <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === tab ? 'bg-admin-primary text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>{count}</span>
        </button>
    );
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-md border-r-4 border-admin-primary">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                ...
                <div className="text-left sm:text-right flex-shrink-0">
                    ...
                    <p className="text-3xl font-bold text-status-delivered">{order.total.toLocaleString()} ج.س</p>
                </div>
            </div>
    ...
    const EmptyState: React.FC<{ title: string, message: string }> = ({ title, message }) => (
         <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
            ...
            <h2 className="mt-4 text-2xl font-bold text-slate-800 dark:text-slate-100">{title}</h2>
        ...
        <>
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md mb-6">
                <div className="flex border-b dark:border-slate-700">
                    <TabButton tab="available" label="الطلبات المتاحة" count={availableOrders.length} />
            ...
            <div className="space-y-6">
                {activeTab === 'available' && (
                    loading.available ? <SpinnerIcon className="w-8 h-8 text-admin-primary animate-spin mx-auto mt-10" /> :
                    availableOrders.length === 0 ? <EmptyState title="لا توجد طلبات متاحة" message="تحقق مرة أخرى قريبًا للحصول على مهام توصيل جديدة." /> :
                    availableOrders.map(order => <OrderCard key={order.id} order={order} isAvailable />)
                )}

                {activeTab === 'mine' && (
                     loading.mine ? <SpinnerIcon className="w-8 h-8 text-admin-primary animate-spin mx-auto mt-10" /> :
                     myDeliveries.length === 0 ? <EmptyState title="لا توجد طلبات للتوصيل" message="يمكنك قبول الطلبات من قسم 'الطلبات المتاحة'." /> :
                     myDeliveries.map(order => <OrderCard key={order.id} order={order} />)
                )}
            </div>
            ...
            <ConfirmationModal
                ...
                title="تأكيد العملية"
                message={`هل تؤكد أنك قمت بتوصيل الطلب #${confirmingOrder?.id.slice(0, 7).toUpperCase()} واستلمت مبلغ ${confirmingOrder?.total.toLocaleString()} ج.س نقدًا؟`}
                confirmText={isSubmitting[confirmingOrder?.id || ''] ? 'جارِ التأكيد...' : 'نعم، أؤكد'}
                cancelText="إلغاء"
            />
        </>
    );
