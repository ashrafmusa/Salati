...
import { getOptimizedImageUrl, calculateStoreProductPrice } from '../utils/helpers';
...
const HomeScreen: React.FC = () => {
    ...
    const allProducts: StoreProduct[] = useMemo(() => [...items, ...bundles], [items, bundles]);

    const productPrices = useMemo(() => {
        const priceMap = new Map<string, number>();
        if (!settings) return priceMap;
        allProducts.forEach(p => {
            priceMap.set(p.id, calculateStoreProductPrice(p, items, settings));
        });
        return priceMap;
    }, [allProducts, items, settings]);
    ...
    useEffect(() => {
        const unsubscribers = [
            db.collection('items').onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
                setItems(snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) } as Item)));
            }),
            db.collection('bundles').onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
                setBundles(snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) } as Bundle)));
            }),
            db.collection('reviews').onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
                setReviews(snapshot.docs.map(doc => doc.data() as Review));
            }),
            db.collection('categories').orderBy('sortOrder').onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
    ...
    const productsWithRatings = useMemo(() => {
        ...
        reviews.forEach(review => {
            ...
            ratingsMap.set(review.productId, { total: existing.total + review.rating, count: existing.count + 1 });
        });

        return allProducts.map(product => {
            const ratingData = ratingsMap.get(product.id);
            return ratingData 
                ? { ...product, averageRating: ratingData.total / ratingData.count, reviewCount: ratingData.count } 
                : product;
        });
    }, [allProducts, reviews]);
    ...
    const filteredProducts = useMemo(() => {
        return productsWithRatings.filter(product => {
            ...
            const matchesRating = activeFilters.rating === 0 || (product.averageRating || 0) >= activeFilters.rating;
    ...
    const noResults = !loading && !noProductsAvailable && filteredProducts.length === 0;
    ...
    return (
        <>
            ...
            <main className="px-4 sm:px-6 py-6 max-w-7xl mx-auto">
                <h1 className="sr-only">منصة سـلـتـي: تسوق البقالة والمزيد عبر الإنترنت</h1>
                <PromotionalBanner />
                ...
                <section className="mt-12">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {categories
                            ...
                            .map(category => (
                            <div key={category.id} onClick={() => handleCategorySelect(category.name)} className="group relative rounded-xl overflow-hidden cursor-pointer transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                ...
                <section ref={allProductsSectionRef} className="mt-12">
