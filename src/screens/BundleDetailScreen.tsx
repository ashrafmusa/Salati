...
import { calculateBundleSdgPrice, getOptimizedImageUrl } from '../utils/helpers';
...
const BundleDetailScreen: React.FC = () => {
  ...
  const bundleBasePrice = useMemo(() => {
    if (!bundle || !settings || !areAllItemsLoaded) return 0;
    return calculateBundleSdgPrice(bundle, allItems, settings);
  }, [bundle, allItems, settings, areAllItemsLoaded]);
  ...
  const handleAddToCart = () => {
    ...
    setIsAdding(true);
    setTimeout(() => {
        // FIX: The addToCart function expects the quantity as the second argument.
        addToCart(bundle, 1, selectedExtras);
        setIsAdded(true);
        setIsAdding(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border dark:border-slate-700">
                <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-2 flex items-center gap-2"><PackageIcon className="w-5 h-5"/> محتويات الحزمة</h3>
                <ul className="space-y-1 list-disc list-inside mr-4">
                  {bundle.contents.map(content => {
                    const item = bundleItems.find(i => i.id === content.itemId);
                    return <li key={content.itemId} className="text-slate-600 dark:text-slate-300">{item?.arabicName} (x{content.quantity})</li>
                  })}
                </ul>
              </div>
              ...
              {availableExtras.length > 0 && (
                <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border dark:border-slate-700">
                  <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-2">إضافات اختيارية</h3>
                  <div className="space-y-2">
                    {availableExtras.map(extra => (
                      <label key={extra.id} className="flex items-center justify-between p-3 rounded-md hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer">
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="mr-3 text-slate-700 dark:text-slate-200">{extra.name}</span>
                        </div>
                        <span className="font-semibold text-slate-500 dark:text-slate-400">+{extra.price} ج.س</span>
                      </label>
                    ))}
                  </div>
                </div>
        ...
        <div className="p-4 mt-8">
          ...
          <div className="space-y-6">
            {reviews.length > 0 ? (
              ...
              <p className="text-center text-slate-500 py-8">لا توجد مراجعات لهذه الحزمة حتى الآن.</p>
            )}
