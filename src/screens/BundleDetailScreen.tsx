...
const BundleDetailScreen: React.FC = () => {
  ...
  const bundleBasePrice = useMemo(() => {
    if (!bundle || !settings) return 0;
    return calculateBundleSdgPrice(bundle, bundleItems, settings);
  }, [bundle, bundleItems, settings]);
  ...
  const isLowStock =
    bundle?.stock !== undefined && bundle.stock > 0 && bundle.stock < 10;
  const averageRating =
    reviews.length > 0
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen">
      <MetaTagManager
        ...
        description={
          bundle.description
            ? bundle.description.substring(0, 160)
            : `شراء حزمة ${bundle.arabicName} بأفضل سعر وجودة عبر منصة سـلـتـي.`
        }
      />
                ...
                <ul className="space-y-1 list-disc list-inside mr-4">
                  {bundle.contents.map((content) => {
                    ...
                    return (
                      ...
                      >
                        {item?.arabicName} (x{content.quantity})
                      </li>
                    );
                  })}
                </ul>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border dark:border-slate-700">
                  ...
                  <div className="space-y-2">
                    {availableExtras.map((extra) => (
                      <label
                        key={extra.id}
                        className="flex items-center justify-between p-3 rounded-md hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer"
                      >
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="mr-3 text-slate-700 dark:text-slate-200">
                            {extra.name}
                          </span>
                        </div>
          ...
          <div className="space-y-6">
            {reviews.length > 0 ? (
              ...
              <p className="text-center text-slate-500 py-8">
                لا توجد مراجعات لهذه الحزمة حتى الآن.
              </p>
            )}
