...
const AdminReportsScreen: React.FC = () => {
  ...
  const salesData = useMemo(() => {
    ...
    const newOrders = deliveredOrders.length;
  ...
  const productPerformance = useMemo(() => {
    ...
    const itemsWithStock = items.map(item => ({ name: item.arabicName, stock: item.stock }));
    const slowestMoving = itemsWithStock
      .filter(item => !productSales.has(items.find(i => i.arabicName === item.name)?.id || ''))
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
            ...
            <div className="flex items-center gap-4">
                <input type="date" name="start" value={dateRange.start} onChange={handleDateChange} className="p-2 border rounded-md bg-white dark:bg-slate-700"/>
                <span>إلى</span>
                <input type="date" name="end" value={dateRange.end} onChange={handleDateChange} className="p-2 border rounded-md bg-white dark:bg-slate-700"/>
                <button onClick={handleExport} className="bg-admin-primary text-white px-4 py-2 rounded-lg hover:bg-admin-primary-hover">تصدير CSV</button>
            </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <StatCard title="الطلبات الجديدة" value={salesData.newOrders.toLocaleString()} />
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md">
                    ...
                    <ul className="space-y-2">
                        {productPerformance.bestSelling.map(item => (
                            <li key={item.name} className="flex justify-between p-2 bg-slate-50 dark:bg-slate-700/50 rounded">
