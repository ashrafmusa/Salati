...
const AdminReportsScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubOrders = ordersQuery.onSnapshot(
      (snapshot) => {
        setOrders(
  ...
  const productPerformance = useMemo(() => {
    ...
    deliveredOrders.forEach((order) => {
      order.items.forEach((cartItem) => {
        const existing = productSales.get(cartItem.productId) || {
          name: cartItem.arabicName,
          quantity: 0,
        };
        productSales.set(cartItem.productId, {
          ...existing,
  ...
  const handleExport = () => {
    ...
    exportToCsv(
      dataToExport,
      `orders_report_${dateRange.start}_to_${dateRange.end}.csv`
    );
  };
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="flex items-center gap-2">
              <input
                ...
                className="p-2 border rounded-md bg-white dark:bg-slate-700 w-full sm:w-auto"
              />
            ...
            <button
              ...
              className="w-full sm:w-auto bg-admin-primary text-white px-4 py-2 rounded-lg hover:bg-admin-primary-hover"
            >
              تصدير CSV
            </button>
          </div>
      ...
      {loading ? (
        <div className="text-center py-10">
          <SpinnerIcon className="w-8 h-8 mx-auto animate-spin text-admin-primary" />
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard
              ...
              value={`${salesData.totalRevenue.toLocaleString()} ج.س`}
            />
            <StatCard
              title="الطلبات المسلمة"
              value={salesData.newOrders.toLocaleString()}
            />
            <StatCard
              title="متوسط قيمة الطلب"
              value={`${salesData.avgOrderValue.toFixed(2)} ج.س`}
            />
          </div>
