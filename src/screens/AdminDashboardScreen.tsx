...
const AdminDashboardScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const ordersQuery = db.collection('orders').where('date', '>=', thirtyDaysAgo.toISOString()).orderBy('date', 'desc');
        unsubs.push(ordersQuery.onSnapshot((snapshot: firebase.firestore.QuerySnapshot) => {
            const orders = snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) } as AdminOrder));
            setAllOrders(orders);
            setLoading(false);
        }));
