...
import { OrdersIcon, BasketsIcon, CustomersIcon, CurrencyDollarIcon } from '../assets/adminIcons';
import { AdminOrder, OrderStatus, Product } from '../types';
...
const RecentOrders: React.FC<{ orders: AdminOrder[], loading: boolean }> = ({ orders, loading }) => (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-4">أحدث الطلبات</h2>
                ...
                <tbody>
                    {loading ? (
                        ...
                        <tr key={order.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <td className="p-3 text-slate-600 dark:text-slate-300">{order.total.toLocaleString()} ج.س</td>
                            <td className="p-3">
                                <span className={`px-3 py-1 text-xs font-semibold rounded-full ${getStatusConfig(order.status).pill}`}>
                                    {order.status}
                                </span>
                            </td>
                        </tr>
            ...
            {loading ? <p>Loading...</p> : orders.map(order => (
                <div key={order.id} className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border dark:border-slate-700">
                    <div className="flex justify-between items-center">
                        ...
                        <p className="font-bold text-primary">{order.total.toLocaleString()} ج.س</p>
                    </div>
                    <div className="mt-2 pt-2 border-t dark:border-slate-600">
                         <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusConfig(order.status).pill}`}>
                            {order.status}
                        </span>
                    </div>
                </div>
