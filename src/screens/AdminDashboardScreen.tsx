...
const AdminDashboardScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const ordersQuery = query(collection(db, 'orders'), where('date', '>=', thirtyDaysAgo.toISOString()), orderBy('date', 'desc'));
        ...
        unsubs.push(onSnapshot(itemsQuery, snapshot => {
            const products = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Item));
            setAllLowStockItems(products.filter(p => (p.stock || 0) < 20).length);
        }));
        
        // Data integrity checks for Super Admins
        if (user?.role === 'super-admin') {
            // FIX: Corrected a typo by adding a space between 'const' and the function name.
            const fetchAllDataForValidation = async () => {
                try {
                    ...
                    const settings = settingsSnapshot.docs.length > 0 ? settingsSnapshot.docs[0].data() as StoreSettings : null;

                    const issues = validateData(items, bundles, settings);
    ...
    const stats = useMemo(() => {
        ...
        const newOrders = filteredOrders.filter(o => o.status === OrderStatus.Preparing).length;
