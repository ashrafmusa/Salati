  ...
  <>
        ...
        <tbody>
          {Array.from({ length: 5 }).map((_, i) => (
            <tr key={i} className="border-b dark:border-slate-700">
              ...
              <td className="p-3">
                <div className="h-6 bg-slate-200 dark:bg-slate-700 rounded-full animate-pulse w-24"></div>
              </td>
            </tr>
          ))}
        </tbody>
    ...
    <div className="space-y-4 md:hidden">
      {Array.from({ length: 3 }).map((_, i) => (
        <div
          key={i}
          className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border dark:border-slate-700 animate-pulse"
        >
    ...
    if (user?.role === "super-admin") {
      ...
      const fetchAllDataForValidation = async () => {
        try {
          ...
          const settings =
            settingsSnapshot.docs.length > 0
              ? (settingsSnapshot.docs[0].data() as StoreSettings)
              : null;
