...
const ItemDetailScreen: React.FC = () => {
    ...
    const isLowStock = item?.stock !== undefined && item.stock > 0 && item.stock < 10;
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen">
            <MetaTagManager 
                ...
                description={item.description ? item.description.substring(0, 160) : `شراء ${item.arabicName} بأفضل سعر وجودة عبر منصة سـلـتـي.`}
            />
            ...
            <div className="pb-40 max-w-5xl mx-auto">
                <div className="p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                        ...
                        <div className="space-y-6">
                            <h1 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-slate-50">{item.arabicName}</h1>
                             {isOutOfStock ? (
                                ...
                                <div className="px-3 py-1 bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300 font-bold rounded-full inline-block">تبقى {item.stock} فقط!</div>
                            )}
                ...
                <RelatedProducts category={item.category} currentProductId={item.id} allItems={allItems} />
                    ...
                    {isAdded ? (
                        ...
                        <div className="flex justify-between items-center gap-4">
                            <div>
                                ...
                                <span className="text-3xl font-bold text-secondary block">{itemPrice.toLocaleString()} ج.س</span>
                            </div>
                            <button onClick={handleAddToCart} disabled={isAdding || isOutOfStock} className="flex-grow sm:flex-grow-0 sm:w-48 px-6 py-3 rounded-lg bg-primary text-white font-bold text-lg flex items-center justify-center transition-colors disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed">
                                {getButtonContent()}
                            </button>
                        </div>
                    )}
