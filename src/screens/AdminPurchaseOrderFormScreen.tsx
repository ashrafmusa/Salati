    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div>
                    <button onClick={() => navigate('/purchase-orders')} className="px-4 py-2 bg-slate-200 rounded-md text-sm mr-2">إلغاء</button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-lg">
                <select name="supplierId" value={po.supplierId || ''} onChange={handleHeaderChange} className="p-2 border rounded-md">
                    <option value="">اختر المورد</option>
                    {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
                <input type="date" name="expectedDate" value={(po.expectedDate || '').split('T')[0]} onChange={handleHeaderChange} className="p-2 border rounded-md" />
                <select name="status" value={po.status || ''} onChange={handleHeaderChange} className="p-2 border rounded-md">
                        ...
                        <tbody>
                            {po.items?.map(item => (
                                <tr key={item.itemId} className="border-b">
                                    ...
                                    <td className="p-2"><input type="number" value={item.quantity} onChange={e => handleItemChange(item.itemId, 'quantity', Number(e.target.value))} className="w-full p-1 text-center border rounded"/></td>
                                    <td className="p-2"><input type="number" value={item.costPrice} onChange={e => handleItemChange(item.itemId, 'costPrice', Number(e.target.value))} className="w-full p-1 text-center border rounded"/></td>
