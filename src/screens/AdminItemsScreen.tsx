...
const AdminItemsScreen: React.FC = () => {
  ...
  const handleToggleFeatured = async (item: Item) => {
    try {
      ...
      logAdminAction(
        adminUser,
        "Toggled Featured Item",
        `Item: ${item.arabicName}, Status: ${
          newStatus ? "Featured" : "Not Featured"
        }`
      );
      showToast(
        `Item is now ${newStatus ? "featured" : "not featured"}.`,
        "success"
      );
    } catch (error) {
  ...
  const handleExport = () => {
    exportToCsv(
      paginatedItems,
      `items_catalog_${new Date().toISOString().split("T")[0]}.csv`
    );
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="w-full sm:w-auto flex flex-col-reverse sm:flex-row items-center gap-2">
            ...
            <button
              ...
              className="w-full sm:w-auto bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200 px-4 py-2 rounded-lg hover:bg-slate-300"
            >
              تصدير CSV
            </button>
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center bg-admin-primary text-white px-4 py-2 rounded-lg hover:bg-admin-primary-hover"
            >
