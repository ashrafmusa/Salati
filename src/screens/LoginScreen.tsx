...
const LoginScreen: React.FC = () => {
      ...
      if (!verifierRef.current && recaptchaContainerRef.current) {
        ...
        const verifier = new firebase.auth.RecaptchaVerifier(
          recaptchaContainerRef.current,
          {
            ...
            "expired-callback": () => {
              setErrors({
                form: "reCAPTCHA expired, please try sending the code again.",
              });
        ...
        verifier.render().catch((err) => {
          ...
          setErrors({ form: "Could not initialize reCAPTCHA." });
        });
      }
    }

    // Cleanup verifier on component unmount or when switching auth method/step
    return () => {
      if (verifierRef.current) {
        verifierRef.current.clear();
        verifierRef.current = null;
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    if (!verifierRef.current) {
      setErrors({
        form: "reCAPTCHA verifier not initialized. Please try again.",
      });
      return;
    }
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        form: err.message || "رمز التحقق غير صالح أو انتهت صلاحيته.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const renderEmailAuth = () => (
    <form onSubmit={handleEmailSubmit} className="space-y-4 animate-form-entry">
      ...
      {mode === "login" && (
        <div className="text-left -mt-2">
          <button
            ...
            className="text-sm font-semibold text-slate-500 hover:text-primary dark:hover:text-primary-light transition"
          >
            هل نسيت كلمة المرور؟
          </button>
        </div>
      ...
      <button
        type="submit"
        className={`${btnPrimaryClasses} mt-6`}
        disabled={loading}
      >
        {loading ? (
          <SpinnerIcon className="w-5 h-5 animate-spin mx-auto" />
          ...
          <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-lg border dark:border-slate-700">
            ...
            <div className="mt-6 text-center text-sm">
              <button
                ...
                className="font-semibold text-slate-500 hover:text-primary dark:hover:text-primary-light transition"
              >
                {mode === "login"
