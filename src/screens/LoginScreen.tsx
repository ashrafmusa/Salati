...
const LoginScreen: React.FC = () => {
  ...
  const { user, loginWithEmail, registerWithEmail, signInWithPhone, verifyOTP, sendPasswordResetEmail } = useAuth();
  ...
  useEffect(() => {
    if (authMethod === 'phone' && recaptchaContainerRef.current && !verifierRef.current) {
        ...
        verifierRef.current = new firebase.auth.RecaptchaVerifier(recaptchaContainerRef.current, {
            'size': 'invisible',
  ...
  const validate = (fieldName?: string): boolean => {
    ...
    if (authMethod === 'phone') {
        if (step === 'phone' && (!fieldName || fieldName === 'phone')) {
            if (!/^\+[1-9]\d{1,14}$/.test(phone)) newErrors.phone = "الرجاء إدخال رقم هاتف صالح بالصيغة الدولية (e.g., +249...).";
        }
        ...
        if (!fieldName || fieldName === 'email') {
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) newErrors.email = "الرجاء إدخال بريد إلكتروني صالح.";
        }
        if (!fieldName || fieldName === 'password') {
            if (password.length < 6) newErrors.password = "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل.";
        }
    }
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    setErrors({});
    try {
        if (!verifierRef.current) throw new Error("Recaptcha verifier not initialized.");
        await signInWithPhone(phone, verifierRef.current);
        setStep('otp');
    } catch (error: any) {
        setErrors({ api: error.message || "فشل إرسال الرمز. الرجاء المحاولة مرة أخرى." });
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleSendResetEmail = async (resetEmail: string) => {
    ...
    showToast(`تم إرسال رابط إعادة تعيين كلمة المرور إلى ${resetEmail}`, 'success');
  };

  const AuthMethodToggle = () => (
    <div className="flex justify-center mb-6">
        <div className="relative flex w-full max-w-xs p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <div className={`absolute top-1 h-10 w-1/2 bg-white dark:bg-slate-700 rounded-md shadow-sm transition-transform duration-300 ease-in-out ${authMethod === 'phone' ? 'translate-x-0' : 'translate-x-full'}`} />
            <button onClick={() => setAuthMethod('phone')} className="relative z-10 w-1/2 py-2 text-center font-semibold">بالهاتف</button>
            <button onClick={() => setAuthMethod('email')} className="relative z-10 w-1/2 py-2 text-center font-semibold">بالإيميل</button>
        </div>
    </div>
  );
  ...
  const FormHeader = () => (
      <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">{mode === 'login' ? 'مرحباً بعودتك!' : 'إنشاء حساب جديد'}</h1>
          <p className="mt-2 text-slate-500 dark:text-slate-400">
              {mode === 'login' ? 'سجل الدخول للمتابعة إلى سـلـتـي.' : 'انضم إلينا اليوم وابدأ التسوق.'}
          </p>
      </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col md:flex-row">
        <div className="hidden md:block md:w-1/2 lg:w-2/5 xl:w-1/3 p-8">
            <div className="h-full flex flex-col justify-between">
                ...
                <img src={illustrationSrc} alt="Shopping illustration" className="w-full max-w-sm mx-auto" />
                <p className="text-center text-slate-500 dark:text-slate-400 text-sm">© {new Date().getFullYear()} Salati. All Rights Reserved.</p>
            </div>
        </div>
                        ...
                        {step === 'phone' && (
                            <div className="space-y-4">
                                <div className="relative">
                                    ...
                                    <input type="tel" dir="ltr" placeholder="رقم الهاتف (e.g., +249...)" value={phone} onChange={(e) => setPhone(e.target.value)} className={`${inputFieldClasses} ${errors.phone ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'}`} />
                                </div>
                                {errors.phone && <p className="text-red-500 text-sm">{errors.phone}</p>}
                                <button type="submit" className={btnPrimaryClasses} disabled={loading}>{loading ? <SpinnerIcon className="w-6 h-6 animate-spin"/> : 'إرسال الرمز'}</button>
                            </div>
                        )}
                        {step === 'otp' && (
                            <div className="space-y-4">
                                <p className="text-center text-slate-600 dark:text-slate-300">أدخل الرمز المكون من 6 أرقام الذي تم إرساله إلى <span dir="ltr" className="font-bold">{phone}</span>.</p>
                                <OTPInput value={otp} onChange={setOtp} />
                                {errors.otp && <p className="text-red-500 text-sm text-center">{errors.otp}</p>}
                                <button type="submit" className={btnPrimaryClasses} disabled={loading}>{loading ? <SpinnerIcon className="w-6 h-6 animate-spin"/> : 'تحقق'}</button>
                                <button type="button" onClick={() => setStep('phone')} className="text-sm text-center w-full mt-2 text-slate-500 hover:text-primary">تغيير الرقم</button>
                            </div>
                        )}
                    </form>
                )}
                    ...
                    <form onSubmit={handleEmailSubmit} className="space-y-4">
                        {mode === 'signup' && (
                            <div className="relative">
                                ...
                                <input type="text" placeholder="الاسم الكامل" value={name} onChange={(e) => setName(e.target.value)} className={`${inputFieldClasses} ${errors.name ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'}`} />
                            </div>
                        )}
                        <div className="relative">
                            <EmailIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                            <input type="email" dir="ltr" placeholder="البريد الإلكتروني" value={email} onChange={(e) => setEmail(e.target.value)} className={`${inputFieldClasses} ${errors.email ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'}`} />
                        </div>
                        <div className="relative">
                            <LockIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                            <input type={showPassword ? 'text' : 'password'} dir="ltr" placeholder="كلمة المرور" value={password} onChange={(e) => setPassword(e.target.value)} className={`${inputFieldClasses} ${errors.password ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'}`} />
                        ...
                        {Object.values(errors).filter(e => e && e !== errors.api).map((err, i) => <p key={i} className="text-red-500 text-sm">{err}</p>)}
                        
                        {mode === 'login' && (
                            <div className="text-right">
                                <button type="button" onClick={() => setIsResetModalOpen(true)} className="text-sm font-semibold text-primary hover:underline">هل نسيت كلمة المرور؟</button>
                            </div>
                        )}
                        
                        <button type="submit" className={btnPrimaryClasses} disabled={loading}>{loading ? <SpinnerIcon className="w-6 h-6 animate-spin"/> : (mode === 'login' ? 'تسجيل الدخول' : 'إنشاء حساب')}</button>
                    </form>
                )}
                
                <p className="mt-8 text-center text-slate-600 dark:text-slate-300">
                    {mode === 'login' ? 'ليس لديك حساب؟' : 'لديك حساب بالفعل؟'}
                    <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setErrors({}); }} className="font-semibold text-primary hover:underline mr-2">
