...
const LoginScreen: React.FC = () => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || `Failed to ${mode}. Please check your credentials.`
      );
    } finally {
      setLoading(false);
    }
  ...
  const isPhoneInvalid = phone.length > 0 && !validatePhoneNumber(phone);
          ...
          <form onSubmit={handlePhoneSubmit} className="space-y-6">
            <div className="relative">
              ...
              <input
                ...
                placeholder="e.g., +249912345678"
                dir="ltr"
                className={`w-full pl-4 pr-12 py-3 rounded-lg border transition bg-slate-50 dark:bg-slate-800 focus:ring-primary focus:border-primary ${
                  isPhoneInvalid || (error && error.includes("الهاتف"))
                    ? "border-red-500"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-primary text-white font-bold py-3 rounded-lg hover:bg-secondary transition-colors disabled:bg-slate-400 flex justify-center items-center"
            >
              {loading ? (
        ...
        return (
          <form onSubmit={handleOtpSubmit} className="space-y-6">
            ...
            <button
              ...
              type="button"
              className="text-sm text-slate-500 hover:underline text-center w-full"
            >
              تغيير الرقم
            </button>
          </form>
    ...
    <form onSubmit={handleEmailSubmit} className="space-y-6">
      {mode === "signup" && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-4 pr-12 py-3 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-primary focus:border-primary transition bg-slate-50 dark:bg-slate-800"
            required
          />
        </div>
      )}
      ...
      <div className="relative">
        ...
        <input
          ...
          dir="ltr"
          className="w-full pl-12 pr-12 py-3 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-primary focus:border-primary transition bg-slate-50 dark:bg-slate-800"
          required
        />
        <button
          type="button"
  ...
  const Tab: React.FC<{
    ...
    <button
      onClick={onClick}
      className={`flex-1 py-3 text-center font-bold transition-colors duration-300 rounded-t-lg border-b-2 ${
        active
  ...
  return (
    <div className="min-h-screen bg-warmBeige dark:bg-slate-950 flex animate-fade-in">
      ...
      <div className="w-full min-h-screen grid grid-cols-1 lg:grid-cols-2">
        <div
          className="hidden lg:flex flex-col items-center justify-center p-12 bg-cover bg-center"
          style={{
            backgroundImage:
              "url('https://res.cloudinary.com/dolmzcken/image/upload/v1756915579/pkvgg5q6i3n1g1vyurcn.png')",
          }}
        >
          <div className="w-full max-w-sm bg-black/30 backdrop-blur-md p-10 rounded-2xl shadow-2xl">
            <Logo imgClassName="w-48" textClassName="text-5xl text-white" />
          </div>
        </div>
