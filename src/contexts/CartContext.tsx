...
import { collection, doc, onSnapshot, query, where, runTransaction, setDoc, getDoc } from "firebase/firestore";
import { useAuth } from "../hooks/useAuth";
import { Product, CartItem, ExtraItem, Offer } from "../types";
import { calculateItemAndExtrasTotal, applyDiscounts } from "../utils/helpers";
...
export interface CartContextType {
  ...
  deliveryFee: number;
  addToCart: (product: Product, selectedExtras?: ExtraItem[]) => void;
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const generateCartId = (product: Product, extras: ExtraItem[]) => {
    const extraKey = JSON.stringify(extras.map((e) => e.id).sort());
    return `${product.id}-${btoa(extraKey).replace(/=/g, '')}`;
  };

  const addToCart = useCallback(
    (product: Product, selectedExtras: ExtraItem[] = []) => {
      ...
      const action = (prevItems: CartItem[]) => {
        ...
        if (index > -1) {
            ...
            newItems.push({
                cartId, id: product.id, name: product.name, arabicName: product.arabicName,
                imageUrl: product.imageUrl, contents: product.contents, category: product.category,
                description: product.description, quantity: 1, selectedExtras,
            });
        }
        return newItems;
      };
  ...
  const getCartSubtotal = useMemo(() => {
    return () => items.reduce((total, item) => total + calculateItemAndExtrasTotal(item, item.selectedExtras) * item.quantity, 0);
  }, [items]);

  const getDiscountDetails = useMemo(() => {
    return () => {
        const { totalDiscount, appliedOfferIds } = applyDiscounts(items, offers);
