    ...
    const mergeAndListen = async () => {
      ...
      if (guestIds.length > 0) {
        try {
          ...
          const firestoreIds =
            docSnap.exists && (docSnap.data() as { itemIds: string[] })?.itemIds
              ? (docSnap.data() as { itemIds: string[] })!.itemIds
              : [];
      ...
      unsubscribe = wishlistRef.onSnapshot(
        (docSnap) => {
          // FIX: Cast document data to correctly access the 'itemIds' property.
          setItemIds(
            docSnap.exists && (docSnap.data() as { itemIds: string[] })?.itemIds
    ...
    async (productId: string) => {
      if (user) {
        ...
        await wishlistRef.set(
          { itemIds: firebase.firestore.FieldValue.arrayUnion(productId) },
          { merge: true }
        );
      } else {
        setItemIds((prevIds) => {
    ...
    async (productId: string) => {
      if (user) {
        ...
        await wishlistRef.update({
          itemIds: firebase.firestore.FieldValue.arrayRemove(productId),
        });
      } else {
        setItemIds((prevIds) => {
