...
const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({ order, onClose, drivers }) => {
    ...
    useEffect(() => {
        if (activeTab === 'log' && order.adminHasUnreadMessages) {
            ...
            }).catch(err => console.error("Failed to mark messages as read for admin:", err));
        }
    }, [activeTab, order.id, order.adminHasUnreadMessages]);

    const TabButton: React.FC<{ tab: ActiveTab, icon: React.FC<{className?:string}>, children: React.ReactNode }> = ({ tab, icon: Icon, children }) => (
        <button onClick={() => setActiveTab(tab)} className={`flex items-center gap-2 px-4 py-3 text-sm font-semibold border-b-2 transition-colors ${activeTab === tab ? 'border-admin-primary text-admin-primary' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>
      ...
      >
        ...
        <div className="border-b dark:border-slate-700 flex-shrink-0">
            <div className="flex items-center gap-2 px-4">
                <TabButton tab="details" icon={OrdersIcon}>تفاصيل الطلب</TabButton>
                <TabButton tab="log" icon={MessageIcon}>السجل والمحادثة</TabButton>
            </div>
        </div>
                ...
                <div>
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg text-slate-800 dark:text-slate-200">
                            ...
                            <p><span className="font-semibold">الطريقة:</span> {order.deliveryMethod === 'pickup' ? 'استلام من المتجر' : 'توصيل للمنزل'}</p>
                                ...
                                <tbody>
                                    {order.items.map(item => (
                                        <tr key={item.cartId} className="border-b dark:border-slate-700">
                                            ...
                                            <td className="p-3 text-slate-600 dark:text-slate-300">{calculateItemAndExtrasTotal(item).toLocaleString()} ج.س</td>
                                            <td className="p-3 text-slate-600 dark:text-slate-300">{(calculateItemAndExtrasTotal(item) * item.quantity).toLocaleString()} ج.س</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="p-6 border-t dark:border-slate-700 flex justify-end items-center sticky bottom-0 bg-white dark:bg-slate-800 z-10">
                        <div className="text-right space-y-1">
                            {order.subtotal !== undefined && ( <div className="text-slate-600 dark:text-slate-300 text-md"><span>المجموع الفرعي:</span><span className="font-semibold mr-2">{order.subtotal.toLocaleString()} ج.س</span></div>)}
                            {order.deliveryFee !== undefined && order.deliveryFee > 0 && ( <div className="text-slate-600 dark:text-slate-300 text-md"><span>رسوم التوصيل:</span><span className="font-semibold mr-2">{order.deliveryFee.toLocaleString()} ج.س</span></div>)}
                            <div className="text-slate-800 dark:text-slate-100"><span className="text-lg">الإجمالي الكلي:</span><span className="text-3xl font-bold text-secondary mr-2">{order.total.toLocaleString()} ج.س</span></div>
                        </div>
                    </div>
                </div>
            ) : (
