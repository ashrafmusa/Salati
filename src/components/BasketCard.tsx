...
// FIX: The `Product` type is obsolete. Switched to the correct `StoreProduct` type.
import { StoreProduct } from '../types';
import { useCart } from '../hooks/useCart';
import { useWishlist } from '../hooks/useWishlist';
import { PlusIcon, HeartIcon, StarIcon } from '../assets/icons';
// FIX: `calculateProductTotal` is obsolete. The component now uses a `price` prop.
import { getOptimizedImageUrl } from '../utils/helpers';

interface ProductCardProps {
  // FIX: Switched from `Product` to `StoreProduct`.
  product: StoreProduct;
  // FIX: Added a `price` prop to display the pre-calculated price.
  price: number;
}
...
const ProductCard: React.FC<ProductCardProps> = ({ product, price }) => {
  ...
  // FIX: Added logic to differentiate between item and bundle links.
  const linkPath = product.type === 'bundle' ? `/bundle/${product.id}` : `/item/${product.id}`;


  const handleAddToCart = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();
    // FIX: The second argument for `addToCart` expects extras, passing an empty array.
    addToCart(product, []);
  };
  ...
  return (
    <Link to={linkPath} className="block bg-white dark:bg-slate-900 rounded-xl shadow-md overflow-hidden group transition-all duration-300 transform hover:-translate-y-1.5 active:translate-y-0 hover:shadow-glow-primary dark:hover:shadow-glow-primary/50 border dark:border-slate-800">
      <div className="relative overflow-hidden">
        <img src={getOptimizedImageUrl(product.imageUrl, 400)} alt={product.name} className="w-full h-40 object-cover transition-transform duration-500 ease-in-out group-hover:scale-110" />
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mt-4">
          {/* FIX: Replaced `calculateProductTotal` with the `price` prop. */}
          <span className="text-2xl font-bold text-primary-dark dark:text-primary font-display">{price.toLocaleString()} ج.س</span>
          <button 
