...
const DataValidationNotice: React.FC<DataValidationNoticeProps> = ({
  issues,
  onDismiss,
}) => {
  if (issues.length === 0) {
            ...
            <ul className="list-disc list-inside space-y-1">
              {issues.map((issue, index) => (
                <li key={index}>
                  ...
                  <Link
                    to={issue.link}
                    className="font-bold underline hover:text-amber-800 dark:hover:text-amber-100 mr-2"
                  >
                    {issue.linkText} &rarr;
                  </Link>
                </li>
