...
const SupplierFormModal: React.FC<SupplierFormModalProps> = ({ supplier, onClose, onSave, isSaving, users, allSuppliers }) => {
    ...
    const availableUsersForLinking = useMemo(() => {
        const linkedUserIds = new Set(allSuppliers.map(s => s.userId).filter(Boolean));
        const currentUserForThisSupplier = supplier?.userId ? users.find(u => u.uid === supplier.userId) : null;
        ...
        const unlinkedCustomers = users.filter(u => u.role === 'customer' && !linkedUserIds.has(u.uid));
    
        if (currentUserForThisSupplier && !unlinkedCustomers.some(u => u.uid === currentUserForThisSupplier.uid)) {
    ...
    const inputClasses = (name: keyof Supplier | 'userId') => `w-full p-2 border rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 ${errors[name] ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'} focus:ring-admin-primary focus:border-admin-primary`;
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                           ...
                           <input id="contactPerson" name="contactPerson" value={formData.contactPerson || ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('contactPerson')} mt-1`} />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                           <label htmlFor="phone" className="block text-sm font-medium">رقم الهاتف</label>
                           <input id="phone" type="tel" name="phone" value={formData.phone || ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('phone')} mt-1`} dir="ltr"/>
                           {errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}
                        </div>
                        <div>
                           <label htmlFor="email" className="block text-sm font-medium">البريد الإلكتروني</label>
                           <input id="email" type="email" name="email" value={formData.email || ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('email')} mt-1`} dir="ltr"/>
                        </div>
                    </div>
                     <div>
                        <label htmlFor="address" className="block text-sm font-medium">العنوان</label>
                        <textarea id="address" name="address" value={formData.address || ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('address')} mt-1`} rows={2} />
                    </div>
                    <div>
                        <label htmlFor="userId" className="block text-sm font-medium">ربط حساب مستخدم (للوصول للبوابة)</label>
                        <select id="userId" name="userId" value={formData.userId || ''} onChange={handleChange} className={`${inputClasses('userId')} mt-1`}>
                     ...
                     <div>
                       ...
                       <input id="paymentTerms" name="paymentTerms" value={formData.paymentTerms || ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('paymentTerms')} mt-1`} />
                    </div>
                     <div>
                        <label htmlFor="bankDetails" className="block text-sm font-medium">التفاصيل البنكية</label>
                        <textarea id="bankDetails" name="bankDetails" value={formData.bankDetails || ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('bankDetails')} mt-1`} rows={2} />
                    </div>
