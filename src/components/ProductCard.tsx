...
const StoreProductCard: React.FC<StoreProductCardProps> = ({ product, price }) => {
  ...
  const isAddToCartDisabled = isOutOfStock || (product.type === 'bundle' && !areAllItemsLoaded);
      ...
      <div className="relative overflow-hidden">
        ...
        {isOutOfStock && (
            <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                <span className="text-white font-bold px-3 py-1 bg-red-600 rounded-full text-sm">نفدت الكمية</span>
            </div>
        )}
        {isLowStock && (
        ...
        <div className="flex justify-between items-center">
            ...
            {product.averageRating && product.reviewCount && product.reviewCount > 0 ? (
                <div className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <span className="font-bold text-slate-600 dark:text-slate-300">{product.averageRating.toFixed(1)}</span>
                    <span>({product.reviewCount})</span>
                </div>
            ) : null}
        </div>
        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 truncate mt-2 group-hover:text-primary-dark transition-colors">{product.arabicName}</h3>
        <div className="flex justify-between items-center mt-4">
          <span className="text-xl sm:text-2xl font-bold text-primary-dark dark:text-primary font-display">{price.toLocaleString()} ج.س</span>
          {cartItem ? (
            <QuantitySelector
                quantity={cartItem.quantity}
                onIncrease={() => { if(cartItem.quantity < product.stock) updateQuantity(cartItem.cartId, cartItem.quantity + 1) }}
                onDecrease={() => updateQuantity(cartItem.cartId, cartItem.quantity - 1)}
            ...
            <button 
                ...
                title={isAddToCartDisabled && !isOutOfStock ? "Loading pricing info..." : ""}
            >
