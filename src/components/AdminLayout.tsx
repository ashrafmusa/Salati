...
const Sidebar: React.FC<{ isOpen: boolean, setIsOpen: (isOpen: boolean) => void }> = ({ isOpen, setIsOpen }) => {
  ...
  const inactiveLinkClasses = 'text-gray-300 hover:bg-admin-sidebar-hover hover:text-white';
  ...
  return (
    <>
      ...
      <aside className={`w-60 sm:w-64 bg-admin-sidebar text-admin-sidebar-text flex flex-col fixed h-full z-40 transition-transform duration-300 ease-in-out md:translate-x-0 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-16 flex items-center justify-between px-4 text-2xl font-bold text-white border-b border-white/10">
          ...
          <button className="md:hidden text-slate-400 hover:text-white" onClick={() => setIsOpen(false)} aria-label="Close sidebar">
                ...
                >
                  {({isActive}) => (
                      <>
                          ...
                          <div className={`absolute right-0 top-0 h-full w-1 bg-admin-primary transition-transform duration-300 ease-in-out ${isActive ? 'scale-y-100' : 'scale-y-0 group-hover:scale-y-75'}`}></div>
                      </>
                  )}
                </NavLink>
              </li>
...
const Header: React.FC<{ onMenuClick: () => void }> = ({ onMenuClick }) => {
    ...
    const currentLink = navLinks.slice().reverse().find(link => 
    ...
    return (
        <header className="bg-white dark:bg-slate-800 shadow-md h-16 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20 border-b dark:border-slate-700">
            <div className="flex items-center gap-3">
                 <button className="md:hidden text-slate-500 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white mr-1" onClick={onMenuClick}>
                ...
                {user && (
                    <div className="flex items-center gap-2 border-r border-slate-200 dark:border-slate-700 pr-2 sm:pr-4">
                        <div className="hidden sm:block text-right">
                            <p className="font-bold text-sm text-slate-800 dark:text-slate-100 truncate max-w-24">{user.name}</p>
