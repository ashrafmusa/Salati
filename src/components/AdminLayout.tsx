...
import { DashboardIcon, OrdersIcon, CustomersIcon, LogoutIcon, MenuIcon, CloseIcon, GiftIcon, TruckIcon, EyeIcon, CategoryIcon, ShieldCheckIcon, BeakerIcon, CogIcon, PackageIcon, UserCircleIcon, ChartBarIcon, ClipboardListIcon, ChevronDoubleLeftIcon, BuildingStorefrontIcon, ClipboardDocumentListIcon, BuildingLibraryIcon } from '../assets/adminIcons';
...
const realEstateNavLinksList: NavLinkItem[] = [
    { to: '/properties', label: 'إدارة العقارات', icon: BuildingLibraryIcon, roles: ['admin', 'super-admin']},
];
...
const Sidebar: React.FC<{
  ...
  const realEstateNavLinks = filterLinksByRole(realEstateNavLinksList);
    ...
    <>
      ...
      <aside className={`bg-admin-sidebar text-admin-sidebar-text flex flex-col fixed h-full z-40 transition-all duration-300 ease-in-out
        ...
        <nav className="flex-grow p-3 overflow-y-auto overflow-x-hidden">
          ...
          {realEstateNavLinks.length > 0 && (
            <div className="mt-4 pt-4 border-t border-white/10">
              <SectionHeader title="العقارات" icon={BuildingLibraryIcon} />
              <ul>{realEstateNavLinks.map((link) => <li key={link.to}><NavItem link={link} /></li>)}</ul>
            </div>
          )}
...
const Header: React.FC<{ onMenuClick: () => void, isDesktopCollapsed: boolean }> = ({ onMenuClick, isDesktopCollapsed }) => {
    ...
    const allLinks = [...mainNavLinksList, ...realEstateNavLinksList, ...supplyChainNavLinksList, ...superAdminNavLinksList];
