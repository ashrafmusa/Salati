...
import { DashboardIcon, OrdersIcon, BundlesIcon, CustomersIcon, LogoutIcon, MenuIcon, CloseIcon, GiftIcon, TruckIcon, EyeIcon, CategoryIcon, ShieldCheckIcon, BeakerIcon, CogIcon, PackageIcon } from '../assets/adminIcons';
...
const navLinks: NavLinkItem[] = [
  { to: '/', label: 'لوحة التحكم', icon: DashboardIcon, roles: ['sub-admin', 'admin', 'super-admin', 'driver'] },
  { to: '/orders', label: 'إدارة الطلبات', icon: OrdersIcon, roles: ['sub-admin', 'admin', 'super-admin'] },
  { to: '/bundles', label: 'إدارة الحزم', icon: BundlesIcon, roles: ['admin', 'super-admin'] },
  ...
  { to: '/items', label: 'إدارة الأصناف', icon: PackageIcon, roles: ['super-admin'] },
...
const Sidebar: React.FC<{ isOpen: boolean, setIsOpen: (isOpen: boolean) => void }> = ({ isOpen, setIsOpen }) => {
  ...
  const linkBaseClasses = 'relative flex items-center px-4 py-3 my-1 rounded-md transition-all duration-200 group overflow-hidden';
  const inactiveLinkClasses = 'text-gray-300 hover:bg-admin-sidebar-hover hover:text-white hover:-translate-x-1';
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-admin-sidebar text-admin-sidebar-text flex flex-col fixed h-full z-40 transition-transform duration-300 ease-in-out md:translate-x-0 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <nav className="flex-grow p-4 overflow-y-auto">
          <ul>
            {user?.role !== 'driver' && (
              <li>
                  <a href="./index.html" target="_blank" rel="noopener noreferrer" className={`${linkBaseClasses} ${inactiveLinkClasses}`}>
                ...
                >
                  {({isActive}) => (
                      <>
                          ...
                          <div className={`absolute right-0 top-0 h-full w-1 bg-white/80 transition-transform duration-300 ease-in-out ${isActive ? 'scale-y-100' : 'scale-y-0 group-hover:scale-y-75'}`}></div>
                      </>
                  )}
                </NavLink>
              </li>
...
const Header: React.FC<{ onMenuClick: () => void }> = ({ onMenuClick }) => {
    const location = useLocation();
    const { user } = useAuth();
    const currentLink = navLinks.slice().reverse().find(link => location.pathname.startsWith(link.to) && link.to !== '/');
    ...
    return (
        <header className="bg-warmBeige/80 dark:bg-slate-900/70 backdrop-blur-sm shadow-sm h-16 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20 border-b dark:border-slate-800">
            <div className="flex items-center">
                 <button className="md:hidden text-slate-500 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white mr-4" onClick={onMenuClick}>
                    <MenuIcon className="w-6 h-6"/>
                </button>
                <h1 className="text-xl md:text-2xl font-semibold text-charcoal dark:text-slate-100">{title}</h1>
            </div>
