...
const PromotionalBanner: React.FC = () => {
    ...
    useEffect(() => {
        const unsubscribe = db.collection('offers').onSnapshot(snapshot => {
    ...
    return (
        ...
        >
            ...
            {banners.length > 1 && (
                <>
                    <button onClick={prevBanner} aria-label="Previous Banner" className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/60 dark:bg-black/60 text-slate-800 dark:text-white p-2 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110 active:scale-95 z-10">
                        <ChevronLeftIcon className="w-6 h-6" />
                    </button>
                    <button onClick={nextBanner} aria-label="Next Banner" className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/60 dark:bg-black/60 text-slate-800 dark:text-white p-2 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110 active:scale-95 z-10">
                    ...
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center space-x-2 z-10">
                        {banners.map((_, index) => (
                            <button
                                ...
                                className={`h-2 rounded-full transition-all duration-500 ease-out ${index === currentIndex ? 'w-5 bg-white' : 'w-2 bg-white/50 hover:bg-white/80'}`}
