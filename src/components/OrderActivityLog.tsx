...
const LogEntry: React.FC<{
    ...
    switch (log.type) {
      case "customer_message":
        return {
          ...
          align: isCurrentUser ? "self-end" : "self-start",
          text: isCurrentUser ? "text-right" : "text-left",
        };
      case "admin_message":
        return {
          bg: isCurrentUser
  ...
  if (log.type === "system_log" || log.type === "status_change") {
    return (
      <div
        className={`text-xs text-slate-500 dark:text-slate-400 py-1 ${align} ${text}`}
      >
  ...
  return (
    <div
      className={`p-3 rounded-lg max-w-xs md:max-w-md ${bg} ${align} ${text}`}
    >
...
const OrderActivityLog: React.FC<OrderActivityLogProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = logQuery.onSnapshot(
      (snapshot) => {
  ...
  const internalLogs =
    userRole === "admin"
      ? logs.filter((log) => log.visibility === "internal")
      : [];
        ...
        {userRole === "admin" && internalLogs.length > 0 && (
          <>
            <div className="text-center text-xs font-bold uppercase text-slate-400">
              --- السجل الداخلي وملاحظات السائق ---
            </div>
            <div className="flex flex-col gap-2">
              {internalLogs.map((log) => (
                <LogEntry key={log.id} log={log} currentUser={user} />
              ))}
            </div>
            <div className="text-center text-xs font-bold uppercase text-slate-400">
              --- المحادثة مع العميل ---
      ...
      <div className="p-4 border-t dark:border-slate-700 bg-white dark:bg-slate-800">
        <div className="flex items-start gap-2">
          <textarea
            ...
            placeholder={
              userRole === "admin" && messageType === "internal"
                ? "أضف ملاحظة داخلية..."
                : "اكتب رسالتك..."
            }
            rows={2}
          ...
          <button
            ...
            disabled={isSending}
            className="p-3 bg-primary text-white rounded-md disabled:bg-slate-400"
          >
            {isSending ? (
