...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const { path, points, gridLines, xLabels, yLabels } = useMemo(() => {
    ...
    const yTickValue = maxValue > 0 ? Math.pow(10, Math.floor(Math.log10(maxValue) || 0)) / 2 : 500;
    const roundedMax = Math.ceil(maxValue / yTickValue) * yTickValue;

    const xScale = (index: number) => padding.left + (index / (data.length > 1 ? data.length - 1 : 1)) * chartWidth;
    const yScale = (value: number) => padding.top + chartHeight - (roundedMax > 0 ? (value / roundedMax) * chartHeight : chartHeight);
      ...
      >
        ...
        <g>
            {gridLines.map((line, i) => (
                <line key={i} {...line} className="stroke-slate-200 dark:stroke-slate-700 animate-fade-in-chart" />
            ))}
            {yLabels.map((label, i) => (
                <text key={i} x={padding.left - 8} y={label.y + 4} textAnchor="end" className="text-xs fill-slate-500 dark:fill-slate-400">
                    {label.label}
                </text>
            ))}
        </g>
        ...
        <g>
            {xLabels.map((label, i) => (
                <text key={i} x={label.x} y={height - padding.bottom + 20} textAnchor="middle" className="text-xs fill-slate-500 dark:fill-slate-400">
                    {label.label}
                </text>
            ))}
        </g>
        ...
        {activePoint && (
          <g className="pointer-events-none animate-fade-in-chart">
            <line x1={tooltipPos.x} y1={padding.top} x2={tooltipPos.x} y2={padding.top + chartHeight} className="stroke-slate-300 dark:stroke-slate-600" strokeDasharray="4 4"/>
            <circle cx={tooltipPos.x} cy={tooltipPos.y} r="5" fill="white" stroke={color} strokeWidth="2" />
          </g>
        )}
      </svg>
      {/* Tooltip */}
