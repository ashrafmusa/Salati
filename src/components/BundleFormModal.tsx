...
const BundleFormModal: React.FC<BundleFormModalProps> = ({
  bundle,
  onClose,
  onSave,
  isSaving,
  ...
  const validate = (field?: keyof Bundle, value?: any) => {
    ...
    if (field === "arabicName" || !field) {
      if (!currentData.arabicName?.trim())
        newErrors.arabicName = "اسم الحزمة مطلوب";
    ...
    if (field === "contents" || !field) {
      if (!currentData.contents || currentData.contents.length === 0)
        newErrors.contents = "يجب أن تحتوي الحزمة على صنف واحد على الأقل";
  ...
  const handleRemoveItem = (itemId: string) => {
    const newContents = (formData.contents || []).filter(
      (c) => c.itemId !== itemId
    );
    setFormData((prev) => ({ ...prev, contents: newContents }));
    validate("contents", newContents);
  ...
  const handleQuantityChange = (itemId: string, quantity: number) => {
    ...
    const newContents = (formData.contents || []).map((c) =>
  ...
  const inputClasses = (name: keyof Bundle) =>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-6 text-slate-800 dark:text-slate-100">
          {bundle ? "تعديل حزمة" : "إنشاء حزمة جديدة"}
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input
            ...
            placeholder="اسم الحزمة بالعربي"
            className={inputClasses("arabicName")}
          />
          {errors.arabicName && (
          ...
          <div className="border-t pt-4">
            ...
            {errors.contents && (
              <p className="text-red-500 text-xs mb-2">{errors.contents}</p>
            )}
            <div className="space-y-2">
              {(formData.contents || []).map((content) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex items-center justify-between bg-slate-50 dark:bg-slate-700/50 p-2 rounded-md"
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-16 p-1 text-center rounded-md"
                      />
                      <button
                        type="button"
