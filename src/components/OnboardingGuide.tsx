...
const TUTORIAL_STEPS: TutorialStep[] = [
  ...
  },
  {
    ...
    title: 'تصفية النتائج',
    content: 'يمكنك تصفية المنتجات حسب السعر أو التقييم للعثور على ما يناسبك تمامًا.',
    position: 'bottom',
    path: '/',
  },
  {
  ...
  },
  {
    ...
    content: 'استخدم شريط التنقل السفلي للوصول بسهولة إلى عربة التسوق، المفضلة، وحسابك.',
    position: 'top',
    path: '/',
  },
  {
    ...
    title: 'إدارة حسابك',
    content: 'من هنا يمكنك تعديل معلوماتك، عرض سجل طلباتك، وإعادة عرض هذا الدليل التعليمي.',
    position: 'top',
    path: '/',
  },
];
...
const OnboardingGuide: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const pollTimeout = setTimeout(() => {
      ...
      if (!document.querySelector(step.selector)) {
        console.warn(`Onboarding element not found: ${step.selector}, skipping.`);
        nextStep();
      }
    }, 3000);
  ...
  }, [currentStep, step.selector, step.path, nextStep, navigate, location.pathname]);
  ...
  const getTooltipPosition = () => {
      ...
      if (pos.left + tooltipWidth > window.innerWidth - margin) pos.left = window.innerWidth - tooltipWidth - margin;
      if (pos.top < margin) pos.top = margin;
      if (pos.top + tooltipHeight > window.innerHeight - margin) pos.top = window.innerHeight - tooltipHeight - margin;
      ...
      <div 
        ...
        style={{
            clipPath: highlightRect 
                ? `path(evenodd, 'M 0 0 H ${window.innerWidth} V ${window.innerHeight} H 0 Z M ${highlightRect.left - 8} ${highlightRect.top - 8} H ${highlightRect.left + highlightRect.width + 8} V ${highlightRect.top + highlightRect.height + 8} H ${highlightRect.left - 8} Z')` 
        ...
        <>
          ...
          >
            ...
            <div className="flex justify-between items-center mt-4">
              <button onClick={finishGuide} className="text-sm text-slate-500 hover:underline">تخطي الدليل</button>
              <div className="flex items-center gap-2">
                <button onClick={prevStep} disabled={currentStep === 0} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50">
                  <ChevronRightIcon className="w-5 h-5" />
                </button>
                <span className="text-sm font-semibold">{currentStep + 1} / {TUTORIAL_STEPS.length}</span>
                <button onClick={nextStep} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
