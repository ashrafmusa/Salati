    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-lg shadow-xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label
              htmlFor="customDeliveryFee"
              className="block text-sm font-medium text-slate-700 dark:text-slate-300"
            >
              رسوم توصيل مخصصة (ج.س)
            </label>
            <input
          ...
          <div className="flex justify-end space-x-4 space-x-reverse pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2 bg-slate-300 dark:bg-slate-600 dark:text-slate-100 rounded-md hover:bg-slate-400 dark:hover:bg-slate-500 transition-all duration-200"
            >
              إلغاء
            </button>
            <button
              type="submit"
              className="px-6 py-2 bg-admin-primary text-white rounded-md hover:bg-admin-primary-hover transition-all duration-200 flex justify-center items-center w-24 disabled:bg-slate-400"
              disabled={isSaving}
            >
              {isSaving ? (
