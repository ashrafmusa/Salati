      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <header className="flex items-center justify-between p-4 border-b dark:border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">AI Bundle Idea Generator</h2>
          </div>
        ...
        <main className="flex-grow p-6 overflow-y-auto">
          {loading && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="mt-4 font-semibold text-slate-600 dark:text-slate-300">Generating creative ideas...</p>
          ...
          {!loading && !error && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {ideas.map((idea, index) => (
                <div key={index} className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border dark:border-slate-700 flex flex-col justify-between animate-stagger-in" style={{ animationDelay: `${index * 100}ms` }}>
                  <div>
                    <h3 className="font-bold text-lg text-primary">{idea.bundleName}</h3>
                    <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">{idea.description}</p>
                    <div className="mt-2">
                        <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mt-2">Suggested Items:</h4>
                        <ul className="list-disc list-inside text-sm text-slate-600 dark:text-slate-400 mt-1 space-y-1">
                            {idea.itemNames.map((itemName, i) => <li key={i}>{itemName}</li>)}
                        </ul>
                    </div>
                  </div>
                  <button
