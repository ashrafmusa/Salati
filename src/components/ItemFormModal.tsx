...
const ItemFormModal: React.FC<ItemFormModalProps> = ({ item, onClose, onSave, isSaving, categories, suppliers }) => {
    ...
    useEffect(() => {
        if (item) {
            ...
            setFormData({
                ...
                description: '', costUSD: 0, markupPercentage: 25, stock: 0, isFeatured: false,
            });
    ...
    const validate = (field?: keyof Item) => {
        ...
        if (field === 'costUSD' || !field) checkField('costUSD', (currentData.costUSD ?? 0) <= 0, "التكلفة يجب أن تكون أكبر من صفر");
        if (field === 'markupPercentage' || !field) checkField('markupPercentage', (currentData.markupPercentage ?? -1) < 0, "هامش الربح لا يمكن أن يكون سالباً");
    ...
    const inputClasses = (name: keyof Item) => `w-full p-2 border rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 ${errors[name] ? 'border-red-500' : 'border-slate-300 dark:border-slate-600'} focus:ring-admin-primary focus:border-admin-primary`;
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <h2 className="text-2xl font-bold mb-6 text-slate-800 dark:text-slate-100">{item ? 'تعديل صنف' : 'إضافة صنف جديد'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                           <label htmlFor="costUSD" className="block text-sm font-medium">سعر التكلفة (USD)</label>
                           <input id="costUSD" type="number" name="costUSD" step="0.01" value={formData.costUSD ?? ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('costUSD')} mt-1`} />
                           {errors.costUSD && <p className="text-red-500 text-xs mt-1">{errors.costUSD}</p>}
                        </div>
                        <div>
                           <label htmlFor="markupPercentage" className="block text-sm font-medium">هامش الربح (%)</label>
                           <input id="markupPercentage" type="number" name="markupPercentage" value={formData.markupPercentage ?? ''} onChange={handleChange} onBlur={handleBlur} className={`${inputClasses('markupPercentage')} mt-1`} />
                    ...
                    <div>
                        ...
                        <div className="mt-2 flex items-center gap-4">
                           <img src={imagePreview ? getOptimizedImageUrl(imagePreview, 200) : 'https://via.placeholder.com/150'} alt="Preview" className="w-24 h-24 rounded-md object-cover shadow-sm" />
                    ...
                    <div className="flex items-center gap-2">
                        <input id="isFeatured" type="checkbox" name="isFeatured" checked={formData.isFeatured || false} onChange={handleChange} className="h-4 w-4 rounded" />
                        <label htmlFor="isFeatured" className="text-sm font-medium">عرض في الصفحة الرئيسية</label>
                    </div>
