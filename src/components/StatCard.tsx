...
const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 transition-transform duration-300 group-hover:scale-105`}
      >
        {React.cloneElement(icon, {
          className: `w-7 h-7 transition-colors ${textColor}`,
        })}
      </div>
      <div>
